...
public:                            
    ...
    unsigned int table_2[256];   // предпросчитаная таблица адресов смещения точек вдоль фокальной линии (упаковано по 2 short) //
    unsigned long table_3[256];  // предпросчитаная таблица адресов точек начала фокальных линий (упаковано по 4 short) //
...
///__________________function declaration__________________//
  ...
  { return new GBitmap(ref,scale,0,0);}
  ...
  { return new GBitmap(ref,0,rotation,0);}
 ...
 /**\brief Constructs a GBitmap by copying and unpack raw bits TIFF data*/
  ...
  // ss "ширина" Гауссиана в пикселях  gaussian
  // TMP test
  ...
  /**\brief  Функция определения габаритов изображения. mode=1 - 
   Убирает белый фон вокруг изображения и возвращает новый GBitmap; mode=0 возвращяет габариты без создания нового изображения 
   frame - отступ от края изображения (белый бордюр) в новом GBitmap.
   */
  ...
  /////
  // заготовка для бикубика на основе scaleFast(), не готова
  ...
  /**\brief функция определения угла наклона текста с помощью быстрого
    ...
    или 2 на 4 (g=24) 
   */
  float detectPageAngle( unsigned int g);
  ...
  // TMP
  /// Версия где росчет матрицы происходит в теле цикла.
