...
int GBitsetOCR::setImageParamOCR(GBitmap *pechaImg_,vector<float*>&param,OCRPoint &point){
...
TIME_START
	...
	for (int index=lineIndex;index>=(int)strArray[0].size()/4; --index){  //for every string
       ...
       ((GImageEditor*)inputData.imageEditor)->adaptiveFiltration1(line,param[0],2);
       setMatrix=GBitsetMatrix::createRegion(line,
        ...
        if(matchLine.size()<10){
            ...
            for(int indexParam=0;indexParam<param.size();indexParam++){
                ...
                ((GImageEditor*)inputData.imageEditor)->adaptiveFiltration1(line,param[indexParam],2);
                setMatrix=GBitsetMatrix::createRegion(line,
                                                      0,
                                                      0,
                                                      line->columns(),
