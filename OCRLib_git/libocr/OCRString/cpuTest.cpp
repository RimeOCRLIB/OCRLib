...
void StartCpuInfo(){
    ...
    DT("CpuType: "<<" Intel="<<Intel<<" AMD="<<AMD<<endl); 
    
    // вывод на экран расшифровки параметра  SIMDType:  SSE=3, SSE2=4, SSE3=5, SSE41=6, SSE42=7, Altivec=8.
    DT("SIMDType: "<<" MMX="<<MMX<<" SSE="<<SSE<<" SSE2="<<SSE2<<" SSE3="<<SSE3<<" SSE41="<<SSE41<<" SSE42="<<SSE42<<" Altivec="<<Altivec<<endl<<endl);             
    ...
    DT("запускаем функцию   setMatrix_Point  -  ");
        if(  g_Cpu.SIMD==MMX ) {                                                             // 1. MMX
            g_Cpu.KeySMP=1;  DT("MMX"<<endl); 
        }              
        if( g_Cpu.SIMD==SSE || g_Cpu.SIMD==SSE2 || g_Cpu.SIMD==SSE3 || g_Cpu.SIMD==SSE41 ) { // 2. SSE или SSE2 или SSE3 или SSE41
            g_Cpu.KeySMP=2;  DT("SSE-SSE41"<<endl); 
        }    
        if( g_Cpu.Popcnt==1 || g_Cpu.SIMD==SSE42 || g_Cpu.SIMD==Altivec ) {                  // 3. SSE42 или Altivec на POPCNT
            g_Cpu.KeySMP=3;  DT("SSE42-Altivec & popcnt"<<endl);  
        }                                 

#ifdef NO_ASM
    if( g_Cpu.SIMD==MMX ) {                                                               // 1. CPU32
        g_Cpu.KeySMP=4;  DT("CPU32"<<endl);
    }
    if( g_Cpu.SIMD==SSE || g_Cpu.SIMD==SSE2 || g_Cpu.SIMD==SSE3 || g_Cpu.SIMD==SSE41 ) {  // 2. CPU64
        g_Cpu.KeySMP=5;  DT("CPU64"<<endl);
    }
    if( g_Cpu.Popcnt==1 || g_Cpu.SIMD==SSE42 || g_Cpu.SIMD==Altivec ) {                   // 3. CPU64 на popcnt intrinsic
        g_Cpu.KeySMP=6;  DT("CPU64 & popcnt intrinsic"<<endl);
    }
#endif
    //#ifdef REMARK
...
const SCpuInfo& GetCpuInfo()
{
    ...
    if (1)
    {
        ...
        if (IsCpuIDBitSet(1,0,0,0, 23, RegECX))  { g_Cpu.Popcnt = 1; }  // Popcnt Bit  
