...
namespace ocr{
    ...
    // Возвращает номер самой стабильной особой (фокальной) точки в массиве векторов focalAdr. Номера точек начинаются с нуля.
    // Работает только с черно-белой картинкой.     
    
    int  GGraph::StablePoint(vector<OCRPoint>&focalAdr) {
        ...
        // Функция возвращает номер самой стабильной особой (фокальной) точки в массиве переменной длинны focalAdr. Номера точек начинаются с нуля.
        ...
        sizeAdr=focalPointXTL(focalAdr);
        if (GrafT) { cout<<"Функция выделения фокальных точек на фокальных линиях букв   focalPoint(focalAdr)   sizeAdr="<<sizeAdr<<endl; }
        /**/
        
        if(!focalAdr.size())return 0;
        ...
        OCRPoint *pt_p=(OCRPoint*)&focalAdr[0];
        // (OCRPoint*) приведение типа
        // focalAdr  массив  OCRPoint-ов
        ...
        /**/
         ...
         /*
         // нахождение номера самой стабильной особой (фокальной) точки в массиве векторов focalAdr. Номера точек начинаются с нуля.
         // лучше применить сортировку адресов в массиве векторов focalAdr по переменной stability.
         int MaxStability=0;   int NoMaxStability=0;
        ...
        // Возвращаем номер самой стабильной особой (фокальной) точки в массиве векторов focalAdr. Номера точек начинаются с нуля.
    ...
    // адресов в массиве векторов focalAdr. Сортировка осуществляется по степени стабильности фокальных точек.
    // Работает только с черно-белой картинкой.     
    
    int  GGraph::StablePointT(vector<OCRPoint>&focalAdr) {
        
        /*
         // Адреса стабильных фокальных точек в сортированном виде (по степени стабильности) расположенны в массиве векторов focalAdr. 
        ...
        /*    
         // Координаты, исчезувших из-за эррозии, фокальных точек заносят в конец массива векторов focalAdr.
         ...
         // - Сравниваются координаты предидущих фокальных точек и новых фокальных точек.
         // - Координаты исчезувших фокальных точек заносят на свободное место в конец массива векторов focalAdr.
...
TIME_START 
        ...
        // dataPtr() возвращает указатель pt_p на реальные данные перепрыгивая через служебные данные (указатель на первый элемент массива )        
         ...
         class OCRPoint {
         public:
         OCRPoint();  short x;  short y;  short status; short stability;
         };
         */
        ...
        if (GrafT) { 
            ...
            /*
             ...
             int s=focalAdr[0][0]->y*w + focalAdr[0][0]->x;
             bytes_data[s]=0;  bytes_data[s+1]=0;  bytes_data[s+w]=0;  bytes_data[s+1+w]=0;
             for ( m=1; m < sizeAdr; m++ ) { bytes_data[focalAdr[0][m]->y*w + focalAdr[0][m]->x]=0; } // читаем из массива векторов
             */
        }
        /**/
    ...
    /*
     ...
     vector<OCRPoint>*focalAdr=vector<OCRPoint>::create(); // вектор
     // запуск функции выделения фокальных точек на фокальных линиях (графемах) букв.
     sizeAdr=focalPoint(focalAdr);
     if (GrafT) { cout<<"Функция выделения фокальных точек на фокальных линиях букв   focalPoint(focalAdr)   sizeAdr="<<sizeAdr<<endl; }
     
     
     // сортировка адресов в массиве векторов focalAdr.
     if (GrafT) {        
     if (n==3) { // n_Lay-1 // n=8
     memcpy(bytes_data_2, bytes_data_1, w_h);
     for ( m=0; m < sizeAdr; m++ ) { bytes_data_2[focalAdr[0][m]->y*w + focalAdr[0][m]->x]=128; } // 0
     } // if (n==n_Lay)
     }
