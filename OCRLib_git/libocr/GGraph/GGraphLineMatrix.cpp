...
TIME_START
        ...
        /*
        uint size=(uint)focalAdr.size();    // Количество линий на странице.
        
        for(int i=0;i<size;i++){
            indexV[(int)focalAdr[i].x]++;  // сколько
        }
...
TIME_START
        ...
        //OCRPoint *test=(OCRPoint*)&line.test[0]; // получение указателя.
...
for (nl=0; nl<NumberLine; nl++){
    ...
    OCRFocalLine & line_nl=focalLine[nl];   // замена focalLine[nl] на line_nl, для увеличения скорости.
    if (line_nl.lenA<LINE_SIZE) continue;   // ограничение на длинну сравниваемой линии N1. LINE_SIZE=6 задаем в GGraph.h
    ...
    for (ny=1; ny<countLine+1; ny++){
        ...
        OCRFocalLine & line_in=focalLine[indexLine]; // замена focalLine[indexLine] на line_in, для увеличения скорости.
