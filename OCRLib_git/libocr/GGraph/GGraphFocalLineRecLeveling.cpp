...
namespace ocr{
    ...
    void GGraph::focalLineNorm( OCRFocalLine &line){
        ...
        */
        ...
        OCRPoint s_line;                            // объявление s_line
      // time=0.039
        line.sData.resize(LengthLine);              // resize
      // time=0.053
        OCRPoint *data=(OCRPoint*)&line.data[0];    // получаем указатель на массив Data
        OCRPoint *sData=(OCRPoint*)&line.sData[0];  // получаем указатель на массив sData
     ...
     // time=0.055
        ...
        // "LINE_SIZE=12" в pix задаем в GGraph.h    //   TRESHOLD_LINE    ithreshold_Line;
     ...
     // time=0.076
        ...
        // Функция для создания таблици определения координат по длине сглаженной фокальной линии.
        focalLineTabM(line);
...
////        float LengthLineM=line.lenM;  // длина сглаженной фокальной линии. LINE_SIZE=6 в pix задаем в GGraph.h
...
////        focalLineBezierP(line, str_L, end_L);
 ...
 /**/ // цифры time без углов и Pg0_pix=7;
    ...
    void GGraph::focalLineMiddleA(OCRFocalLine &line, uint K_Mid2){
        ...
        OCRPoint *sData=(OCRPoint*)&line.sData[0];    // указатели на массивы интегральных сумм точек фокальной линии.
        OCRPoint *mData=(OCRPoint*)&line.mData[0];    // указатели на массивы адресов точек сглаженной фокальной линии.
...
//K_Mid2=1;
        ...
        for ( index=1; index<LengthLine_; index++ ) {
            ...
            // необходимо если длинна фокальной линии LengthLine меньше полной величины усреднения 2*K_Mid2
            if (index<k) k=index;
        ...
        */
        ...
        OCRPoint *data=(OCRPoint*)&line.data[0];    // указатели на массивы интегральных сумм точек фокальной линии.
        ...
        float α=0.28; // чем меньшее значение, тем больше сглаживание 0.4    0.28
        int n=2; // 2
        float mx0, my0, mx1, my1;
        /*
        line.mTab.resize(LengthLine);                // резервируем размер resize.
        OCRPoint *mTab=(OCRPoint*)&line.mTab[0];    // получаем указатель mTab.
 ...
 //       data[LengthLine_].x=end_x;    data[LengthLine_].y=end_y;        // координаты конца фокальной линии.
        ...
        // в конце линии не обработана одна точка LengthLine_-1   n=1
            ...
            // m[i] = m[i-1] + α * (x[i] - m[i-1])      // y[i] = y[i-1] + α * (x[i] - y[i-1])
            mData[index].x=mx1=mx0 + α*(x1-mx0);   mData[index].y=my1=my0 + α*(y1-my0);
            //mTab[index].x=mx1=mx0 + α*(x1-mx0);   mTab[index].y=my1=my0 + α*(y1-my0); // нужен другой массив m[i]=MData
        ...
        */
        ...
        // в начале линии не обработана одна точка index=0; n=1
            ...
            // m[i] = m[i-1] + α * (x[i] - m[i-1])
            mData[index].x=mx1=mx0 + α*(x1-mx0);   mData[index].y=my1=my0 + α*(y1-my0);
            //mTab[index].x=mx1=mx0 + α*(x1-mx0);   mTab[index].y=my1=my0 + α*(y1-my0);
    ...
    //*****************************************************************************
                    ...
                    /// СРЕДНЕЕ ///
    // Функция для сглаживания фокальной линии. Основная рабочая версия. Предидущая версия.
    
    void GGraph::focalLineMiddleB(OCRFocalLine &line, uint K_Mid2){
    ...
    void GGraph::focalLineLengthM(OCRFocalLine &line){
        ...
        OCRPoint *mData=(OCRPoint*)&line.mData[0];
        ...
        /*
         ...
         rg0=index;
         table_Sq[index]=0; // заполнение таблици нулями.
 ...
 //vector<float>sLine;    // вектор для массива интегральных сумм расстояний между соседними точками сглаженной фок.линии (NO Б.Д.).        
    ...
    void GGraph::focalLineTabM(OCRFocalLine &line){
        ...
        // Без таблици, для вычисления каждой координаты, приходится применять линейное чтение массива sLine.
        ...
        if (GrafT) {
            cout<<endl<<"----------------------------------------------------------------------------"<<endl;
            cout<<endl<<"Функция для подсчета таблици определения координат по длине фокальной линии.   focalLineTabM() "<<endl;
            cout<<"Длинна фокальной линии  LengthLine="<<LengthLine<<endl;
        }
        ...
        int mTab_size=LengthLine*3; // обычно, после нормализации, массив почти не меняет свой размер и никогда не может превысить *3.
        line.mTab.resize(mTab_size);                // резервируем размер resize.
        OCRPoint *mTab=(OCRPoint*)&line.mTab[0];    // получаем указатель mTab.
        if(GrafT)cout<<"     line.mTab.size()="<<line.mTab.size()<<endl<<endl;
        
        memset((char*)mTab, 0, mTab_size*8);        //заполняем нулями весь массив.
        ...
        // целочисленный (округленный до целого) ключ для выборки из таблици и его предидущее значение.
        int  indexL, indexL_old;
        ...
        for ( index=0; index<LengthLine; index++ ) { // index=1;
            ...
            OCRPoint & mTab_in0=mTab[indexL];       // указание компилятору на замену mTab[indexL] на mTab_in0, для увеличения скорости.
            ...
            if (indexL==indexL_old) { // если целая часть двух соседних ячеек одинакова.
                ...
                OCRPoint & mTab_in1=mTab[indexL_old];
            ...
            if (indexL>indexL_old+1) { // если индекс двух соседних ячеек отличается больше чем на единицу.
                ...
                OCRPoint & mTab_in2=mTab[indexL_old+1];
        ...
        // все ячейки таблици можно умножить на коэффициент k_LineT почти равной единице.
        ...
        */
        ...
        // Создание таблици //
        // Создание таблици выполняется с помощью линейной интерполяции, пересчетом существующих расстояний и координат
    ...
    */
    ...
    void GGraph::focalLineAngleM(OCRFocalLine &line){
    ...
    // Функция для сглаживания (интерполяции) несколько крайних pix фокальной линии. В разработке.
    void GGraph::focalLineEnds(OCRFocalLine &line, int strL, int endL){
    ...
    // новой кривой Безье..
        ...
        if(line.index==1774) { //  || line.index==5095)
            GrafT=1;
        }else GrafT=0;
...
//        }
        ...
        // int strL - длинна начального участока фокальной линии в pix, не участвующего в вычислении параметров кривой Безье.
        // int endL - длинна конечного участока фокальной линии в pix, не участвующего в вычислении параметров кривой Безье.
...
/*
 ...
 void dotGainArray(OCRFocalLine &line, uint K_Mid2);
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    void GGraph::dotGainArray(OCRFocalLine &line, uint K_Mid2){
        ...
        OCRPoint *data=(OCRPoint*)&line.data[0];      // исходный массив.
        OCRPoint *sData=(OCRPoint*)&line.sData[0];    // массив интегральных сумм.
        OCRPoint *mData=(OCRPoint*)&line.mData[0];    // готовый растиснутый массив.
        ...
        for ( index=1; index<LengthLine_; index++ ) {
            ...
            // необходимо если длинна массива LengthLine меньше полной величины усреднения 2*K_Mid2.
            if (index<k) k=index;
