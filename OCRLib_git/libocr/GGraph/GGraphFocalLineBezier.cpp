...
namespace ocr{
    ...
    void GGraph::focalLineBezierP(OCRFocalLine &line, int strL, int endL){
        ...
        OCRPoint *mTab=(OCRPoint*)&line.mTab[0];
        ...
        if(strL==0 && endL==0) { // 0,0 по полной длине.
            ...
            OCRPoint & mTab_strL=mTab[strL];
            OCRPoint & mTab_endL=mTab[endL];
        ...
        OCRPoint & mTab_t13=mTab[t13];
        OCRPoint & mTab_t131=mTab[t13+1];
        ...
        OCRPoint & mTab_t23=mTab[t23];
        OCRPoint & mTab_t231=mTab[t23+1];
        ...
        */
        ...
        // Использыется для тестирования операции "РЕВЕРС" в функции GGraphCorrelationBezierFull.cpp
        ...
        */
        ...
        line.p0_с.x=(Px0-Px3)/2.0;        line.p0_с.y=(Py0-Py3)/2.0;
        line.p3_с.x=(Px3-Px0)/2.0;        line.p3_с.y=(Py3-Py0)/2.0;
    ...
    void GGraph::focalLineBezierF(OCRFocalLine &line){
    ...
    void GGraph::BezierLength(OCRFocalLine &line){
        ...
        ///OCRPoint *mData=(OCRPoint*)&line.mData[0]; // Для тестового вывода растеризованной кривой Безье.
    ...
    void GGraph::BezierLengthApproximate(OCRFocalLine &line){
    ...
    void GGraph::BezierRasterisation(OCRFocalLine &line, vector<OCRPoint>&lineBezierPoints){
    ...
    void GGraph::recursiveBezier( vector<OCRPoint>&lineBezierPoints,
                                float x1, float y1,
                                float x2, float y2,
                                float x3, float y3,
                                float x4, float y4,
    ...
    void GGraph::focalLineRecMass(OCRFocalLine &line){
        ...
        float sumLine=0;    // суммарная длинна всех расстояний от концов фокальной линии до центров масс.
        ...
        int LengthLine=(int)line.data.size(); // длинна фокальной линии.
        // получаем прямые массивы
        OCRPoint *data=(OCRPoint*)&line.data[0];
        OCRPoint *sData=(OCRPoint*)&line.sData[0];
...
if(GrafT)for(int i=0;i<LengthLine;i++)cout<<i<<" x="<<data[i].x<<"   инт.суммы по x="<<sData[i].x<<endl;
        ...
        // LengthLine длинна для массива, реальная число pix=LengthLine.
...
/////////        line.alpha=(int)alpha; // - alpha, угол наклона линии в градусах (0 до 360° grade).
        ...
        // суммарная длинна расстояний от начала и конца фокальной линии до центра масс.
        ...
        if (LenLinOdd==0) { // четная длинна фокальной линиии.
            if(GrafT)cout<<"Четная длинна фокальной линиии  LenLinOdd="<<LenLinOdd<<endl;
        ...
        else{ // НЕ четная длинна фокальной линиии.
            if(GrafT)cout<<"НЕ четная длинна фокальной линиии  LenLinOdd="<<LenLinOdd<<endl;
        ...
        // суммарная длинна расстояний от начала и конца отрезка "A" до его центра масс.
        sumLine=StartMass1 + EndMass1;
        // Вычисление коэффициента корреляции дуги. Чем ближе коэффициент корреляции к 100, тем больше кривая похожа на дугу.
        ...
        // суммарная длинна расстояний от начала и конца отрезка "B" до его центра масс.
        ...
        // суммарная длинна расстояний между центрами масс отрезков и центром масс всей фокальной линиии Mass0.
   ...
   //     line.corS=(int)cor_Spi;    // - cor_Spi, коэффициент корреляции линии со спиралью (0-100%).
        ...
        if(line.data.size()<LINE_SIZE) { line.corL=100; line.maxC=100; line.maxCor=LINE; } // GGraph.h // <9 <12
    ...
    void GGraph::focalLineRecCircle2 (OCRFocalLine &line) {
        ...
        float dx,dy;
        // Просто линна фокальной линии, т.е. длинна фокальной линии состоящая из всех pix (диагональных и ортогональных).
        ...
        // Истинная длинна фокальной линии круга диаметра равного diameter.
        ///
        ...
        // Полная длинна фокальной линии.
        LengthLineDirect++; // +1 т.к. считаем от середины конечных точек линии 0.5pix + 0.5pix=1pix
        
        // Реальная длинна фокальной линии, состоящая из pix состыкованных под 90° или 0° с длинной в 1 pix, и pix состыкованных под 45°
        // с длинной в 1.41 pix (длинна диагонали pix).
...
void GGraph::LineAngleLength(float delta_x, float delta_y, float &alpha, float &length ){
