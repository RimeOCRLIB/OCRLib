...
void GLogicProcessor::collectTextLine(vector<OCRMatch>&dLine){
    /*
        int ln=(int)matchLine.size(); int maxW=0, indexMaxW;
        ...
        for(int m=0;m<ln;m++){
            ...
            if(matchLine[m].name=="་"){
                ...
                DT2("/tserto/__yLimit1"<<yLimit1<<" maxY="<<matchLine[m].yCenter<<" correlation="<<matchLine[m].correlation<<END);
                if(abs((int)(LimY0*scale)-matchLine[m].yCenter)>
            ...
            if(matchLine[m].OCRIndex==3&&matchLine[m].name!="ུ"&&matchLine[m].name!="ཱ"&&matchLine[m].name!="ཱུ"&&matchLine[m].name!="་"&&matchLine[m].name[0]!='_'&&matchLine[m].name!="ೀ"){
                //если огласовка не на месте мы её гасим
                if((int)(LimY0*scale)-matchLine[m].yCenter>matchLine[m].letterH*3||  //1.5
                   (int)(LimY0*scale)-matchLine[m].yCenter<matchLine[m].letterH*0.3){
                    matchLine[m].correlation=5;
                    matchLine[m].status=1;  //исключаем из рассмотрения
                }
                DT2("/vowel/__LimY0="<<LimY0*scale<<" maxY="<<matchLine[m].yCenter<<" correlation="<<
            ...
            DT2("matchLine["<<m<<"].correlation="<<matchLine[m].correlation<<
                " name="<<matchLine[m].name<<"     xMax="<<m-borderMatrix<<"   y="<<matchLine[m].yCenter<<" w="<<matchLine[m].letterW<<" in="<<matchLine[m].OCRIndex<<END);
        }
        ...
        while(1){  //продолжаем до полной разборки строки
            ...
            //  если эти буквы являются частью буквы мы их гасим
            x0=indexMaxW-maxW/1.7; if(x0<0)x0=0;
            x1=indexMaxW+maxW/1.7; if(x1>ln)x1=ln;
            ...
            for (int xInd=x0;xInd<x1;xInd++){
                ...
                DT2(END<<"/0/______________________________check c="<<matchLine[xInd].status<<" xInd="<<xInd<<" name="<<matchLine[xInd].name<<
                    " correlation="<<matchLine[xInd].correlation<<" OCRIndex="<<matchLine[xInd].OCRIndex<<END);
                ...
                if(matchLine[indexMaxW].OCRIndex==3){
                    if(
                       matchLine[xInd].OCRIndex==3){
                        if(!(matchLine[indexMaxW].name!="་"&&
                             matchLine[xInd].name=="་")){ //огласовки не гасят церто
                            if(matchLine[xInd].correlation<matchLine[indexMaxW].correlation){
        ...
        for(int m=0;m<matchLine.size();m++){
            if(matchLine[m].correlation<50)continue;
            ...
            if(matchLine[m].OCRIndex==3&&matchLine[m].name!="་"){
                ...
                if(m-matchLine[m].letterW/1.7<indexLetter){
                    ...
                    int position=m+1;
                    while(position<ln){
                        if(matchLine[position].correlation<50){
                            matchLine[position]=matchLine[indexLetter];
                            break;
                        }
                        position++;
                    }
        ...
        for(int m=0;m<matchLine.size();m++){
            ...
            if(matchLine[m].correlation>50){
                ...
                //aliKali[0][matchLine[m].letterIndex]->OCRStatus=1000; //помечаем букву как букву из этой книги
                count=0;
                //теперь внутри буквы мы убираем лишние квадратики
                //и устанавливаем верные габариты буквы
                DT2("/@/_______m="<<m<<" ="<<matchLine[m].name<<" matchLine[m].maxY="<<matchLine[m].yCenter<<" yLimit0="<<yLimit0<<END);
                
                //cout<<"m="<<m-borderMatrix<<" matchLine[m].xCenter="<<matchLine[m].xCenter-borderMatrix<<" name="<<matchLine[m].name<<endl;
                x0=(matchLine[m].xCenter-matchLine[m].letterW/2)/scale;//+aliKali[0][matchLine[m].letterIndex]->dX;
                if(x0<0)x0=0;
                x1=(matchLine[m].xCenter+matchLine[m].letterW/2)/scale;//+aliKali[0][matchLine[m].letterIndex]->dX;
                if(x1>ln)x1=ln;
                    ...
                    if(strArray[0][index].wordArray[in].center.x>x0&&
                       ...
                       strArray[0][index].wordArray[in].name=="*"){
                        ...
                        if(count==0){
                            ...
                            strArray[0][index].wordArray[in].y0=matchLine[m].yCenter-matchLine[m].letterH/2+strArray[0][index].y0-LimY0;
                            strArray[0][index].wordArray[in].y1=matchLine[m].yCenter+matchLine[m].letterH/2+strArray[0][index].y0-LimY0;
                            wIndex=in;
                ...
                if(count==0){ //не найдено подходящих квадратиков, нужно вставить в строку новый
                    ...
                    wP.center.y=matchLine[m].yCenter/scale;
                    ...
                    wP.y0=matchLine[m].yCenter-matchLine[m].letterH/2+strArray[0][index].y0-LimY0;
                    wP.y1=matchLine[m].yCenter+matchLine[m].letterH/2+strArray[0][index].y0-LimY0;
        ...
        if((correlation+correlation/10<bestCorrelation&&correlation<65)||correlation<50){
        ...
        for(int a=0;a<lineText.size();a++){
            if(lineText[a].OCRIndex==3&&a>0&&lineText[a-1].OCRIndex==10)continue;  //в морфемах огласовки уже расставленны
