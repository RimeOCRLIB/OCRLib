...
GBitsetMatrix::init(const GBitmap *ref, short stepW, short stepH, short flipV)
{
	  ...
	  int dH,dW;
	  int h=ref->rows();
	  drawPoint=GGraph(ncolumns,nrows);
...
void GBitsetMatrix::init(const GBitmap *ref,
	  ...
	  int dH,dW;
	  int refH=ref->rows();
      drawPoint=GGraph::create(ncolumns,nrows);
...
if(flipV){
    ...
    uchar* t=0;
    //каждый бит в байте цвета обозначает наличие в этом пикселе соотвоетсвуюшего вида фокальной точки
    //для вариабельности каждая форальная точка занимает круговую область 32 пиксела вокруг начальных координат
