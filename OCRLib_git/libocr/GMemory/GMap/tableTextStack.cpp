...
void GMap::tableTextStack(string&text){
    ...
    //BufET_vector=GStr<uint>::create();          // массив (int) для хранения адресов перевода каретки Enter текста
    //BufUpT_vector=GStr<ushort>::create();      // массив для восстановления исходного кода первой буквы пары    
    // из упакованного (компактного) кода пар букв текста
    //BufUpT1_vector=GStr<uchar>::create();     // тоже что и массив BufUpT но состоит из иэ char 0 или 255 и служит для вычисление Max длины 
    ...
    //*****массивы для функции lookupProcessFontMatch3****************************
    ...
    if ( flagPSymbols==OCR_DICT_NO_DELIMETERS) {   // "1" символы пробела, точки в словаре и тексте игнорируются   
    ...
    for(x=0; x < text_size1; x++) {
        wstring st; st=(wchar_t)resultText[x]; DR(Unicode_to_UTF(st))
    }
#endif    
    ...
    DR(endl<<"вывод первой буквы пары"<<endl)
    unsigned short m;   
    nEnterT=0;              
    olds=0; s=text_data[0];   // "olds" предыдущее значение  "s"
    uint d;
    int text_size_1=text_size-1;
...
///        DR(" "<<m<<" "<<d<<"   ");  DR("+++  ")
    ...
    BufTxt_vector->resize(text_size);
    BufTxt=(ushort*)BufTxt_vector->dataPtr();
    size_BufTxt=text_size;
    
#ifdef REMARK    
    // вывод на экран распознаваемого текста text_data (упакованные пары букв), шрифтом, UTF-16 (short)
    DR(endl<<"@@@BufUpT - ПЕРВАЯ буква распознаваемого текст text_data (упакованные пары букв), шрифтом, UTF-16 (short)"<<endl)
    s=0;
    for(x=0; x < text_size1; x++){
