...
void GMap::tableLettersFontMatch(){
    
    // создание структуры для работы с адресами пар букв в распознаваемом тексте
    ...
    //cout<<" letterX_.size()="<<letterX_.size()/2<<" letters_data_vector->size()="<<letters_data_vector->size()<<endl;
    //exit(0);
    ...
    DR(endl<<"вывод в  table_Letters ПЕРВОЙ буквы распознаваемого текста text_data, массив BufUpT, шрифтом, UTF-16 (short)"<<endl)
    s=0;
    for(x=0; x < text_size1/graph; x++){ //text_size1 размер массива распознаваемого текста без переводов каретки Enter
        str=(wchar_t)BufUpT[text_data[x]];
...
TIME_START
    ...
    // Самодельная структура предназначена для получения адресов пар букв в распознаваемом тексте (м.б. в словаре) по компактному коду пары букв,
...
// определение параметров (размерности) таблицы адресов букв в тексте:
