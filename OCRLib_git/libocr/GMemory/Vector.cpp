...
void Vector::reloadPtr(){
    ...
    vectorID		=&innerData[7]; 	 	  //индех записи, в которой Vector размещен в родительском векторе
};

/**инициализация Vector с размещением в оперативной памяти*/
Vector::Vector(){
    data=(char*)malloc(1024*24);
    dataLocation=MEMORY_LOCATION; 	 	 	 	//флаг размещения данных в оперативной  или  отображаемой памяти
    innerData=(uint*)(data);
    innerData[0]=0xfffffffa; 	 	 	 	 	//маркер наличия Vector
    reloadPtr();
    *poolSize=1024*24;
...
Vector::Vector(cstr path){
    ...
    //проверяем является ли файл файлом Vector
    if(dataMFile->size()==0){//новый файл Vector
        ...
        *recordCount=1;
        ...
        if(innerData[0]!=0xfffffffa){
            //cout<<"no valid Vector file "<<path; return;
        }
        reloadPtr();
        index=(uint*)(data+*indexOffset);  //инициализация индекса
        *poolSize=(uint)dataMFile->size();
...
/**инициализация Vector указателем на буфер данных Vector*/
Vector::Vector(char* data_,int mode){
    data=data_;
...
/**инициализация Vector копированием с размещением в оперативной памяти*/ /// ####
...
Vector::Vector(Vector& parentVector, cstr name){
    ...
    //проверяем является ли файл файлом Vector
    innerData=(uint*)(data);
    if(innerData[0]!=0xfffffffa){//новый Vector
        ...
        *lockFlag=0;
        ...
        //проверяем является ли файл файлом Vector
        //cout<<"reload Vector"<<endl;
        innerData=(uint*)(data);
        if(innerData[0]!=0xfffffffa){
            //cout<<"no valid Vector file "; return;
        }
        reloadPtr();
        index=(uint*)(data+*indexOffset);  //инициализация индекса
        *vectorID=vectorID_;
...
/*
void Vector::operator = (Vector *ref){
...
void Vector::addRecords(Vector &ref){
...
Vector::~Vector(){
    cout<<"call destructor "<<*recordCount<<endl;
    if (dataLocation==MEMORY_LOCATION){
        std::free(data);
    }else{
        delete dataMFile;
...
void* Vector::operator[] (uint indexRecord){
    return (void*)(data+index[indexRecord+1]+4);
};

//добавление новой записи в Vector
void Vector::push_back(char*ptr,uint size){
    //data=dataMFile->data();
    ...
    if(*recordArraySize<*recordCount+1){
        //cout_<<"resize Vector index"<<" recordArraySize="<<*recordArraySize<<" recordCount="<<*recordCount<<" *poolSize="<<*poolSize<<endl;
        ...
        if((*recordArraySize)*INDEX_RESIZE+*dataSize>*poolSize){
            //cout<<"start resize for index. New size="<<*poolSize+(*recordArraySize)*INDEX_RESIZE+size*FILE_RESIZE_DELTA<<endl;
	   	    ...
	   	    if(*vectorID==0xffffffff){
                ...
                parent->putPtr(vectorID_,data,*poolSize);
                data=(char*)parent->getPtr(vectorID_,&newSize);
                //cout_<<"@new size="<<newSize<<endl;
	        }
    ...
    if(*poolSize<*dataSize+size+256){
        ...
        //cout<<"@resize Vector file new poolSize="<<newSize/n<<" Mb c3"<<endl;
...
void Vector::push_back(const char*str){
    push_back((char*)str,(int)strlen(str));
};

void Vector::push_back(string&str,cstr name){
    push_back(str);
    string strName;
    getName(strName);
...
void Vector::push_back(TStr &st){
    string str;
    uint markTStr=0xfffffffc;
    str.resize(st.sizeData+st.len*4+12);
    memcpy(&str[0],(char*)&markTStr,4);
...
void Vector::push_back(VStr<int> &v){
    push_back(v.dataPtr(), *v.poolSize);
}


/** resize Vector for new capacity*/
void  Vector::resizeData(uint recordSize, uint datasize){
    ...
    //cout_<<"@resize Vector file new poolSize="<<newSize<<endl;
    uint vectorID_=*vectorID;      //сохраняем vectorID от потери при перезаписи указателей
...
/** resize Vector for new record count*/
void  Vector::resize(uint newRecordCount){
    //cout_<<"*poolSize="<<*poolSize<<" newSize="<<newSize<<endl;
    //cout_<<"@resize Vector file new poolSize="<<newSize<<endl;
    if(*recordCount<newRecordCount){
        unsigned long newSize=(newRecordCount*8+*poolSize)/1000000;
...
void* Vector::getPtr(uint indexRecord,uint *size){
...
void Vector::getStr(uint indexRecord, string&str){
    indexRecord++;  //пропускаем индекс именных записей
    uint *size;
...
void Vector::getStr(uint indexRecord, vector<int>&str){
    indexRecord++;  //пропускаем индекс именных записей
    uint *size;
...
void Vector::getStr(string&str,cstr name){
    uint *size;
    string name_=name;
    uint indexRecord=0;
...
int Vector::getInt(uint indexRecord){
    indexRecord++;  //пропускаем индекс именных записей
    int size;
    if(indexRecord>*recordCount){size=0;return NULL;}
    size=*((uint*)(data+index[indexRecord]));
...
void Vector::getTStr(uint indexRecord,TStr &str){
    indexRecord++;  //пропускаем индекс именных записей
...
void Vector::getName(string&str){
    uint *size;
...
void Vector::putPtr(uint indexRecord,void* ptr, uint size){
    indexRecord++;  //пропускаем индекс именных записей
    uint *sizeRecord;
    int print=0;
    //if(inputData.start){
...
void Vector::putTStr(uint indexRecord, TStr &st){
    string str;
    uint markTStr=0xfffffffc;
    str.resize(st.sizeData+st.len*4+12);
    memcpy(&str[0],(char*)&markTStr,4);
...
void Vector::putName(string &str){
    uint *sizeRecord;
    ...
    if(*sizeRecord >=size){
        ...
        *sizeRecord=size;
        ...
        //проверяем достаточно ли места для новой записи
        if(*poolSize-1024<*dataSize+size){
            //cout<<"resize Vector file new poolSize="<<(*poolSize)*FILE_RESIZE+size*FILE_RESIZE_DELTA<<"c2"<<endl;
...
void Vector::putStr(uint indexRecord, string&str){
 	putPtr(indexRecord,&str[0],(uint)str.size());
};
void Vector::putStr(uint indexRecord, vector<int>&str){
    putPtr(indexRecord,&str[0],(uint)str.size()*4);
};
void Vector::putCStr(uint indexRecord,const char* str){
 	putPtr(indexRecord,(void*)str,(uint)strlen(str));
};
void Vector::putInt(uint indexRecord,int data){
 	putPtr(indexRecord,(void*)&data,4);
};

char* Vector::setVector(cstr name,uint *vectorID){
...
/**возврат указателя на дочерний VStr или Vector по именной записи */
void* Vector::getVector(cstr name){
    //проверяем, существует ли такая запись
    string name_=name;
    uint indexRecord=0;
    string strName; getName(strName);
...
/**возврат указателя на дочерний VStr или Vector по ID */
void* Vector::getVector(uint indexRecord){
    //проверяем, существует ли такая запись
    indexRecord++;
    if(indexRecord!=0){
        char *dataPointer=data+index[indexRecord]+4;
...
void Vector::drawHTML(vector<uint>&searchResult,string &result,int mode){
    ostringstream str;
    int a=0;
    //cout_<< "startRec="<<startRec<<" rowsNum="<<rowsNum<<endl;
...
void Vector::drawCatalogHTML(vector<uint>&searchResult,Vector*vectorLibPath,string &result,int mode){
    ostringstream str;
    int a=0; 
    string pathName;
    string pathLink;
...
/**вывод в result HTML Vector* strResult */
void Vector::drawHTML(uint startRec,int rowsNum,Vector& strResult,string &result,int mode){
...
void Vector::drawHTML(vector<uint>&searchResult,Vector& strResult, string &result,int mode){
    ostringstream str;int a=0;
    uint rowsNum=25;
    if (rowsNum>searchResult.size())rowsNum=(uint)searchResult.size();
    
    for(int index=0;index<rowsNum;index++){
        TStr strDict; getTStr(searchResult[index],strDict);
        string dictResult; strResult.getStr(a,dictResult);
        //cout_<<"strDict.size="<<strDict.size;
        //for(int i=0;i<strDict.len;i++)cout<<" "<<strDict[i]<<endl;  cout<<endl;
        if(!strDict.size()){continue;}
...
void Vector::import(string &path, int mode){
...
void Vector::importTXT(string &path){
...
void Vector::importDict(map<string,uint>keyMap,string &path,string &path1){
    char *textBuffer, *startIndexPtr;
    char c;
    long int sizeText=readInMemory(&textBuffer,path);
    //cout<<"sizeText="<<sizeText<<endl;
...
void Vector::importTAB(string &path){
    char *textBuffer, *startIndexPtr;
    char c;
    long int sizeText=readInMemory(&textBuffer,path);
    //cout<<"sizeText="<<sizeText<<endl;
...
void Vector::importXML(string &path){
	...
	//в Vector в записи все поля записаны как TStr
	//название поля кодируются порядком расположения в TStr
...
void Vector::exportTStr(cstr path){
    string str,line;
...
/** экспорт всех записей формата TStr*/
void Vector::exportTStr(string &path){
    exportTStr(path.c_str());
}

//экспорт в XML в формате FileMaker
void Vector::saveXML(string &path){
...
TStr::TStr(void){
    ...
    data=dataStr.dataPtr();
    index=(uint*)indexStr.dataPtr();
};

TStr::~TStr(void){};

void TStr::push_back(char*ptr, uint size_){
    dataStr.push_Ptr(ptr,size_);
    indexStr.push_back(dataStr.size());
...
void TStr::push_back(string &str){
    push_back((char*)&str[0],(uint)str.size());
};

void TStr::push_back(const char* str){
    push_back((char*)str,(uint)strlen(str));
};

void TStr::push_TabStr(string &str){
...
void TStr::operator +=(string &str){
    push_back((char*)&str[0],(uint)str.size());
};

void TStr::operator +=(cstr str){
    push_back((char*)str,(uint)strlen(str));
};

void TStr:: operator +=(uint i){
    push_back((char*)&i,4);
}

string TStr::operator [](uint i){
    string str;
...
int TStr::readInt(uint i){
   int dataInt;
   memcpy(&dataInt,data+index[i],4);
   return dataInt;
}

void TStr::readStr(string &str, uint i){
    uint size_=index[i+1]-index[i]; 
    str.resize(size_);
    memcpy(&str[0],data+index[i],size_);
}

void TStr::readIntVector(vector<int>&intVector,uint i){
    uint size_=index[i+1]-index[i];
    intVector.resize(size_/4);
    memcpy(&intVector[0],data+index[i],size_);
}

void TStr::readCharVector(vector<char>&charVector,uint i){
        uint size_=index[i+1]-index[i];
        charVector.resize(size_);
        memcpy(&charVector[0],data+index[i],size_);
}

void TStr::setFromPtr(uint*index_,uint len_,char*data_,uint dataSize){
    index=index_;
    data=data_;
    len=len_;
    sizeData=dataSize;
