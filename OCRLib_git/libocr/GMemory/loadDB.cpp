...
void GMemory::importOCR(string &name,string &path,string &delimeter){
    ...
    for(int i=0;i<fileList.size();i++){
        ...
        text=str_replace("¶¶","¶",text);
        text=str_replace("¶¶","¶",text);
        vector<string>t=explode(delimeter,text);
        TString st;
        out.str("");
        out<<"[[ID:"<<timeStamp<<"]]";
        str=out.str();
...
void GMemory::importTextFolder(string &name, string &path, string &category){
    ...
    for(int i=0;i<fileList.size();i++){
        ...
        text=str_replace("¶¶","¶",text);
        //split on pages
        vector<string>lines=explode("¶",text);
        vector<string>t;
        str="";
        int n=0;
        while(n<lines.size()){
...
void GMemory::importDictTab(string &name, string &path,string &category,int year){
    ...
    for(int i=0;i<fileList.size();i++){
        ...
        for(int n=0;n<lines.size();n++){
            ...
            vector<string>t=explode("\t",lines[n]);
    ...
    if(!rec.index[indexAddress].status){
        ...
        switch(searchMode){
            ...
            case FULL_TEXT_SEARCH:{
                ...
                selfPath=out_.str();
                out_.str("");
                out_<<inputData.data["DATA_DB"]<<rec.name<<"/"<<rec.name<<"_"<<searchField<<"_"<<searchMode<<"_data.index";
                path=out_.str();
                GVector *dictTextData=GVector::create(path);
                out_.str("");
                out_<<inputData.data["DATA_DB"]<<rec.name<<"/"<<rec.name<<"_"<<searchField<<"_"<<searchMode<<"_dict.index";
                path=out_.str();
...
void GMemory::loadDB(){
    ...
    if(inputData.data["loadMode"]=="OCR"){
        ...
        indexRec=createIndex(table[path],5,LETTER_SEARCH);
