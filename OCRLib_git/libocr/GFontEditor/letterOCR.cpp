...
void GFontEditor::rebuildMaskInLetter(GLetter *letter,int* pfds){
	...
	*/
    ...
    GBitmap *img=aliKali->drawPict(letter->letterIndex);
    //WriteImageData(img,"/1_.jpg",0);
    
    c2=setCorrelation(img,letter,1,0);                        //проверяем самокорреляцию

    int count=0, testMaskCount;
    int minCorrelation, minX0,minY0,maskId;
    ...
    DR("@/___letter "<<index<<" "<<letter->name.c_str()<<" selfCorrelation="<<letter->selfCorrelation<<" codeSpace="<<letter->codeSpace<<" maskCount="<<letter->mask32Count()<<" letter->onOffRate="<<letter->onOffRate<<endl)
...
#define DT(x) if(print)cout<<x;
    ...
    GLetter *letter;
    vector<maskData>letterDataLine;
    ...
    string dataStr; dataStr.resize(letterDataLine.size()*sizeof(maskData));
    memcpy(&dataStr[0], &letterDataLine[0], letterDataLine.size()*sizeof(maskData));
