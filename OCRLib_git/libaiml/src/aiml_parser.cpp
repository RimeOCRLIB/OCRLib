 ...
 * public functions
 */
bool AIMLparser::parse(const std::string& filename, bool _trim_blanks, int mode_, bool at_runtime) {
  // init vars
  ...
  cout<<"parse AIML file "<<filename<<endl;  
...
void AIMLparser::endElement(const std::string& name) {
  ...
  if (level == LEVEL_OTHER) {
    if (name == "aiml") { }
      ...
      AIMLentry entry;  //cout<<"patt="<<patt<<" that="<<that<<" topic="<<topic<<endl;
      clean_pattern(that); clean_pattern(topic); clean_pattern(patt);
      if(mode==MEMORY_LOCATION){
          ...
          entry.that.push_back(strip(that));
          entry.topic.push_back(strip(topic));
          entry.patt.push_back(strip(patt));
      }
      entry.templ = templ;
      // _DBG_CODE(msg_dbg() << "PAT[" << patt << "] THAT[" << that << "] TOPIC[" << topic << "]" << endl);
        if(dbOnDisk){
             graphmaster.addEntryToGMap(memoryVector,entry,insert_ordered);
        }else
    ...
    if (name == "template") {
      ...
      if (name == "javascript"){verbatim = false;}
...
bool AIMLparser::dumpMemory(string& path, GVector *memoryVector){
    
        xmlTextWriterPtr writer;
        bool ret = true;
        size_t  chars_read,chars_read_tag;
        size_t len,int_type;
        ...
        try {
            ...
            if (xmlTextWriterStartElement(writer, BAD_CAST("aiml")) < 0) throw string("start userset error ");
            ...
            for(int i=0;i<memVectSize;i++) {
                ...
                key=node->key;
                if(!node->key.size())continue;
                cReadBuffer d(node->templ);
                chars_read_tag = 0;
                string value;
                
                if (!(chars_read = d.readNumber(len))) return false;
                chars_read_tag += chars_read;
                if (!(chars_read = d.readNumber(int_type))) return false;
                chars_read_tag += chars_read;
                ...
                if (xmlTextWriterStartElement(writer, BAD_CAST("category")) < 0) throw string("category error");
                if (xmlTextWriterWriteElement(writer, BAD_CAST "pattern",
                                              BAD_CAST (key.c_str()))<0)throw string("pattern error");
                
                if (xmlTextWriterWriteElement(writer, BAD_CAST "template",
                                              BAD_CAST (value.c_str()))<0)throw string("pattern error");
                
                if (xmlTextWriterEndElement(writer) < 0) throw string("end category error");

            }
            
            if (xmlTextWriterEndDocument(writer) < 0) throw string("end aiml error");
        }
