...
void aiml::tokenizeToList(const string& input, std::list<std::string>& out, bool cant_be_empty, const char* str) {
  string token;
    for (size_t i = 1; std_util::gettok(input, token, i); i++){
        //if(token=="*")token=".";
        out.push_back(token);
    }    
...
void aiml::to_uppercase(string& text) {
  //for (string::iterator it = text.begin(); it != text.end(); ++it) *it = toupper_UTF(*it);
  text=toupper_UTF(text);  
    
}

void aiml::to_lowercase(string& text) {
  //for (string::iterator it = text.begin(); it != text.end(); ++it) *it = tolower_UTF(*it);
  text=tolower_UTF(text);  
}
...
void aiml::do_split(string input, vector<string>& out, const string& sentence_limit, bool do_fitting) {
  ...
  while(!should_end) {
    ...
    //@@@@
    if (pos == string::npos||pos>input.size()-1) { should_end = true; sentence = input; }
    ...
    if (pos == string::npos||pos>input.size()-1) break;
...
void aiml::do_pattern_fitting(string& input) {
  ...
  //  if (!isalnum(*it)) (*it) = ' ';
  //  else (*it) = toupper_UTF(*it);
  //}
  //input=cleanSentence(input);
  input=toupper_UTF(input);
