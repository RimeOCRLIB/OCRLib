...
bool cUserManager::load(const std::string& file) {
  ...
  if (!doc) return true;   //create new file
  ...
  try {
    ...
    ostringstream osstr;
    osstr << exc.line;
      runtime_error = exc.line;
      runtime_error+= " (at line " + osstr.str() + ")";
    ret = false;
  }
  catch(...) {
...
bool cUserManager::save(const std::string& file) {
  ...
  try {
    ...
    cCore::UserMap& user_map = core.user_map;
    for(cCore::UserMap::const_iterator it = user_map.begin(); it != user_map.end(); ++it) {
      ...
      if (xmlTextWriterWriteAttribute(writer, BAD_CAST("name"), BAD_CAST(it->first.c_str())) < 0) throw string("user attribute error");
        ...
        //string topic="test",name="restName";
        //map<string,string> map_=it->second.vars_map;
        //map_[topic]=name;
        //string strVar=dampMap(map_);
      for(StringMAP::const_iterator var_it = var_map.begin(); var_it != var_map.end(); ++var_it) {
        ...
        //if (xmlTextWriterStartElement(writer, BAD_CAST("var")) < 0) throw string("start var error");
        //if (xmlTextWriterWriteCDATA(writer, BAD_CAST(strVar.c_str())) < 0) throw string("set content error");
        //if (xmlTextWriterEndElement(writer) < 0) throw string("end var error");
