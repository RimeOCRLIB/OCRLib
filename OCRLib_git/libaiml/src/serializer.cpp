...
void cWriteBuffer::writeToString(std::string& str) const{
   size_t len = data.size();
   str.resize(len);
   memcpy(&str[0], &data[0], len);
}
...
bool cWriteBuffer::readFromFile(std::istream& file) {
  ...
  data.resize(len);
  if(file.read(&data[0], len))return 1;
  
  return 0;
}

bool cWriteBuffer::readFromString(std::string& str_) {
    size_t len=str_.size();
    if (len == 0) return false;
    data.resize(len+1);
    memcpy((char*)&data[0], (char*)&str_[0], len);
    data[len]='\0';
    return true;
}
...
size_t cReadBuffer::readNumber(size_t& num) {
  ...
  //cout<<" size="<<size<<" pos="<<pos<<" num="<<num<<endl;
  pos += sizeof(size_t);
  return sizeof(size_t);
}
...
bool cReadBuffer::at_end(void) const {
   ...
   if(pos == size-1)return 1;   //краевой эффект
...
bool aiml::writeNumber(ostream& file, size_t n) {
  size_t n2 = htonl(n);
    if(file.write(reinterpret_cast<const char*>(&n2), sizeof(size_t)))return 1;
    return 0;
}

bool aiml::writeString(ostream& file, const string& s) {
  if (!aiml::writeNumber(file, s.length())) return false;
    if(file.write(s.c_str(), s.length())) return 1;
  return 0;
}
...
bool aiml::readString(istream& file, string& out) {
  ...
  char* buf = new char[len+1];
  bool ret = (bool)file.read(buf, len);
  buf[len] = '\0';
