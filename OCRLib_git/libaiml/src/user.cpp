...
const string& cUser::setVar(const string& key, const string& value) {
  ...
  if (final_value.empty()) {
    if (it != vars_map.end()) vars_map.erase(it);
      ...
      if(key=="abuse"||key=="mood"){
          return setMood(key, value);
      }
...
const string& cUser::setMood(const string &key,const string &value){
    if(key=="abuse"){   //mimimetr
        if(value=="@INCREASE@"){
            if(vars_map[key].size()<12){
                vars_map[key]+="@";
            }
            return vars_map[key];
        }
        if(value=="@DECREASE@"){
            if(vars_map[key].size())vars_map[key].resize(vars_map[key].size()-1);
            return vars_map[key];
        }
        
    }
    if(key=="mood"){   //mimimetr
        if(value=="@INCREASE@"){
            if(vars_map[key].size()<12)vars_map[key]+="@";
            return vars_map[key];
        }
        if(value=="@DECREASE@"){
            if(vars_map[key].size())vars_map[key].resize(vars_map[key].size()-1);
...
const std::string cUser::dumpVar(){
    string str;
    std::map<string,string>::iterator it;
    for (it = vars_map.begin(); it != vars_map.end(); it++){
