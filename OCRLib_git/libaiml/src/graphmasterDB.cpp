...
bool cGraphMaster::getAnswerDB(string& input, string& output) {
    ...
    //int print=1;
    DM("input str="<<input<<" size="<<input.size()<<endl);
    ...
    if(dbOnDisk){
        ...
        DM("searchResult="<<searchResult.size()<<endl);
        ...
        DM("searchResult1="<<searchResult.size();)
        if(searchResult.size())DM(" searchResult[0]="<<searchResult[0]<<endl);
...
bool cGraphMaster::parseSearchResult(string &input,
    ...
    if(ms.memBase<100){
        ...
        for(int i=0;i<searchResult.size();i++){
            gNode *node=gNode::create(memData,(uint)searchResult[i]);
            if(node->key.size()>input.size()+10){node->destroy(); continue;}
            //DM("key="<<node->key<<endl;)
            if(node->type==NODE_PATT){
                sortData s; s.ID=(uint)searchResult[i];
    ...
    for(int i=0;i<sort_.size();i++){
        ...
        if(new_s.length()!=input.length()){
            DM("@choice  sort_["<<i<<"]."<<sort_[i].ID<<"="<<key<<" ->"<<sort_[i].path<<endl);
            ...
            ms.templ=node->templ;
            DM(" size="<<node->templ.data.size()<<endl);
            ...
            DM("topic="<<avatar->getVar("topic")<<endl);
...
/*
 ...
 DM("topic="<<user.getVar("topic")<<endl);
