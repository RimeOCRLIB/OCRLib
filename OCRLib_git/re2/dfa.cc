...
DFA::DFA(Prog* prog, Prog::MatchKind kind, int64 max_mem)
  ...
  if (kind_ == Prog::kLongestMatch) {
    nmark = (uint)prog->size();
    start_unanchored_ = prog->start_unanchored();
  }
  nastack_ = 2 * (uint)prog->size() + nmark;
  ...
  int one_state = sizeof(State) + ((uint)prog_->size()+nmark)*sizeof(int) +
  ...
  q0_ = new Workq((uint)prog->size(), nmark);
  q1_ = new Workq((uint)prog->size(), nmark);
