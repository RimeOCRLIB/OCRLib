...
namespace ocr{
    ...
    // сответственно GGraphBezierFocalLine.cpp  и  GGraphBezierfocalPointXTL.cpp . Не забыть очистиь от черного 2х pix бордюр
    // вызовом функции img->eraseBorder(2, 2);
    
    int  GGraphBezier::focalLineRecognition(vector<OCRPointFloat>&focalPoint, vector<OCRFocalLineBezier> &focalLine) {
        
        /// Комментарии к функции ///
        
        // Адреса начальных и конечных фокальных точек функция получает в виде int расположенны в массиве векторов focalPoint,
        ...
        // Помещаем центр маски 3*3 на стартовую особую точку. Маской, с помощью таблицы rg_tab2,
        ...
        // Принцип и конкретная реализация работы матрицей (маской) обсчета (3*3-1) по таблице описана в функции GGraphBezierFocalPoint.cpp
...
TIME_START
        ...
        short dl_nl;    // Координата точек начала фокальных линий (переменная таблицы rg_tab3), “nl” номер стартовой точки.
        ...
        OCRPointFloat p_line;  // для промежуточных точек вдоль фокальной линии. point of the line
        OCRPointFloat p_start; // для особых точек как начало фокальной линии.
        ...
        uint sizeAdr=(uint)focalPoint.size(); // количество особых точек по всему графическому тексту. SumSpecialPoints
...
for (np=0; np<sizeAdr; np++){
    ...
    x0=focalPoint[np].x;  // [-32768, 32768]
    y0=focalPoint[np].y;
    ...
    for (nl=0; nl<nLinePoint; nl++){
        ...
        OCRFocalLineBezier lineP;     // создаем новый объект "линия".
        ...
        for ( index=0; rg_tab2!=0 && index<MaxLengthLine; index++ ) {
            ...
            // Ответ таблицы int table_2 с двумя вариантами смещения маски на dl1 или dl2.
            rg_tab2=table_2[rg0];
            ...
            if ( rg_tab2!=0) { // условие особой точки rg_tab2==0.
                ...
                // распакованная координата входящей и выходящей точки по шаблону матрицы (центральная точка забита 1)
                /*
                if (GrafT){  // if (1) {
                    // текущая координата точки по шаблону матрицы (центральная точка d1 = 0 или 2)
                    ...
                    //  распакованная координата входящей и выходящей точки по шаблону матрицы (центральная точка забита 0)
                ...
                */
                ...
                // Заполнение таблицы (массива) занятости адресов промежуточных точек фокальной линии.
        ...
        //newPoints->resize(0);
        focalLineSplit(newPoints,focalLine,focalPoint,lineP);
        
        //focalLineNorm(lineP);
        // focalLineNorm выполняется в функции GGraphBezierFocalLineRecognition1.
  ...
  /**/
        ...
        if (GrafT) {   // внутренний формат программы 0-белый 255-черный
            ...
            cout<<"суммарная длина фокальных линий, по всему графическому тексту  SumLengthLine="<<SumLengthLine<<endl;
            // визуализация серым особых точек (фокальных точек)
            ////for ( n=0; n < sizeAdr; n++ ) { bytes_data[focalPoint[0][n]->y*w+focalPoint[0][n]->x]=128; } // читаем из массива массивов векторов
            
            // визуализация серым распознанных фокальных линий
            int focalLine_size; // длина фокальной линии
            int SumLengthLine_s=0; // суммарная длина фокальной линии
            ...
            cout<<"суммарная длина фокальных линий (из массива БД), по всему графическому тексту  SumLengthLine_s="<<SumLengthLine_s<<endl;
...
//g9=*(p0)&1, g2=*(p0+1)&2, g3=*(p0+2)&4, g8=*(p0+w)&8, g4=*(p0+w+2)&16, g7=*(p0+w+w)&32, g6=*(p0+w+w+1)&64, g5=*(p0+w+w+2);
    ...
    void GGraphBezier::focalLineRecAngle(vector<OCRPointFloat>&dataVector, GStr<int>*newPoints, uint deltaAngle){
        ...
        OCRPointFloat *data=(OCRPointFloat*)&dataVector[0];
        ...
        // "LINE_SIZE=12" в pix задаем в GGraphBezier.h
...
/**/
    ...
    void GGraphBezier::focalLineSplit(GStr<int>*newPoints, vector<OCRFocalLineBezier> &focalLine,vector<OCRPointFloat>&focalPoint,OCRFocalLineBezier &lineP){
        ...
        if(count){
            ...
            for (int i=0; i<count; i++ ) {
                ...
                if(i==0){
                    ...
                    line.data.resize(d);
                    memcpy((char*)&line.data[0],(char*)&lineP.data[0],d*sizeof(OCRPointFloat));
                    ...
                    int l=d-d1;
                    line.data.resize(l);
                    memcpy(&line.data[0],(char*)&lineP.data[0]+sizeof(OCRPointFloat)*d1,l*sizeof(OCRPointFloat));
                    
                    line.start.type=P_POINT;
                    focalPoint.push_back(line.start);
                }
            ...
            memcpy((char*)&line.data[0],(char*)&lineP.data[0]+sizeof(OCRPointFloat)*start,l*sizeof(OCRPointFloat));
            line.start.type=P_POINT;
            focalPoint.push_back(line.start);
...
void GGraphBezier::buildOCRLineTable(){
    
    // Функция создает 3 предпросчитанные таблицы (заполняет 3 глобальных массива, объявленые в GBitmap.h):
    ...
    for (int index=0; index < 256; index++){
        ...
        // АЛГОРИТМ подсчета особых точек. Заполнение таблицы table_1 //
        // Точно такой же как в GGraphBezierFocalPoint.cpp . При изменениях синхронизировать.
        ...
        // (GrafT) // вывод на экран стыков типа "Т", "Х" и "L_End" по шаблону матрицы (параметр соответственно ==1, ==2, ==4)
        ...
        // АЛГОРИТМ вычисления адресов промежуточных точек.  Заполнение таблицы table_2  //
        ...
        if ( b<7 && b>1 && a01==2 && a10==2 && table_1[index]==0 ) { // и не особая точка // Условие заполнения таблицы. 5
            ...
            *(short*)&rg_tab2=dl_1;   *((short*)&rg_tab2+1)=dl_2;
            ...
            // (GrafT) // распакованная координата входящей и выходящей точки по шаблону матрицы (центральная точка забита 1)
            if (GrafT) {
                // тест
                // Роспаковка координат из table_2 (int) в два знаковых short с двумя вариантами смещения маски на dl1 или dl2.
        ...
        if ( table_1[index]!=0 ) { // если особая точка, то заполняем таблицу.
            ...
            *(short*)&rg_tab3=dl_1;   *((short*)&rg_tab3+1)=dl_2;   *((short*)&rg_tab3+2)=dl_3;   *((short*)&rg_tab3+3)=dl_4;
            ...
            // (GrafT)
            // распакованные координаты начала фокальных линий, выходящих из особых точек, по шаблону матрицы (центральная точка забита 1)
            /*
            if (0) {
    ...
    char table_4[256];   // глобольный, заявленв GGraphBezier.
    =table_4[index]=0;
    char rg4;
    // АЛГОРИТМ вычисления тангенциальных углов промежуточных точек.  Заполнение таблицы table_4  //
    ...
    */
    ...
    void GGraphBezier::buildOCRAlphaTable(){
        
        // Функция создает 2 предпросчитанные таблицы (заполняет 2 глобальных массива float ):
