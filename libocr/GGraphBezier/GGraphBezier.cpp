...
OCRLineRef::OCRLineRef(OCRFocalLineBezier &ref,uint len_){
    index=ref.index;
    alpha=ref.alpha;
...
//________OCRFocalLineBezier_________________
    
OCRFocalLineBezier::OCRFocalLineBezier(){
...
void OCRFocalLineBezier::setDimension(int dx,int dy){
...
void OCRFocalLineBezier::writeToStr(TString &st){
    ...
    string str;
    str.resize(8*sizeof(OCRPointFloat));
    OCRPointFloat *v=(OCRPointFloat*)&str[0];
    v[0]=start;
    v[1]=end;
    v[2]=center;
    v[3]=centerM;
    ...
    st.push_back(str);
    st.push_back((char*)&data[0],(uint)data.size()*(int)sizeof(OCRPointFloat));
    st.push_back((char*)&sData[0],(uint)sData.size()*(int)sizeof(OCRPointFloat));
    st.push_back((char*)&mData[0],(uint)mData.size()*(int)sizeof(OCRPointFloat));
...
void OCRFocalLineBezier::readFromStr(TString &st){
    string str=st[0];
    int *d1=(int*)&str[0];
    string str1=st[1];
    ...
    if(st.size()>2){
        string keyStr=st[2];
        OCRPointFloat *v=(OCRPointFloat*)&keyStr[0];
        start=v[0];
        end=v[1];
        center=v[2];
        centerM=v[3];
    ...
    if(st.size()>3){
        string keyStr=st[3];
        data.resize(keyStr.size()/sizeof(OCRPointFloat));
    ...
    if(st.size()>4){
        ...
        sData.resize(keyStr.size()/sizeof(OCRPointFloat));
        memcpy((char*)&sData[0],(char*)&keyStr[0],keyStr.size());
    }
    if(st.size()>5){
        string keyStr=st[5];
        //mData.push_back(keyStr);
        mData.resize(keyStr.size()/sizeof(OCRPointFloat));
...
void OCRFocalLineBezier::printToScreen(){
...
GGraphBezier::GGraphBezier(int acolumns, int arows){
    colorMode = 1;
...
GGraphBezier::GGraphBezier(int acolumns, int arows,int mode){
    if(mode==BITMAP_32){
...
GGraphBezier::GGraphBezier(const GBitmap* ref){
    if (this != ref)
    {
        border=0;
...
GGraphBezier::GGraphBezier(const GBitmap* ref, int x0,int y0, int w, int h){
 ...
 */   
    unsigned char *p;
    unsigned char *refData=ref->bytes_data;
    int refW=ref->columns();
    //int refH=ref->rows();
    if (this != ref)
    {
...
GGraphBezier::GGraphBezier(const GBitmap* ref, float scale, float rotation,int mode){
...
GGraphBezier::GGraphBezier(string &inputData,int invert_){
