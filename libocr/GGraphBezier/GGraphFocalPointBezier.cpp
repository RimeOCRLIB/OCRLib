...
namespace ocr{
    ...
    int  GGraphBezier::focalPointXTL(vector<OCRPointFloat>&focalPoint) {
        ...
        */
        ...
        OCRPointFloat pt; // обьявление локальной переменной
    ...
    int  GGraphBezier::graphFocalPoint(vector<OCRPointFloat>&focalPoint) {
        ...
        for (int index=0; index < 256; index++){ 
            rg0=index;
            table_1[index]=255; // заполнение единицами всех байт и бит таблицы
            // разборка байтового регистра на отдельные 8 бит
            d9=rg0 & 1;    d2=rg0>>1 & 1; d3=rg0>>2 & 1; d8=rg0>>3 & 1; 
            d4=rg0>>4 & 1; d7=rg0>>5 & 1; d6=rg0>>6 & 1; d5=rg0>>7 & 1;            
            // АЛГОРИТМ //                    
        ...
        for ( x=0; x < w_h_; x++ ) {
            ...
            if (*(pw + x)!=0) {  // d1=*(p0+w+1);    // в 2.8 быстрее 
                ...
                rg0=(*(p0)&1) + (*(p0+1)&2) + (*(p0+2)&4) + (*(p0+w)&8) + (*(p0+w+2)&16) + (*(p0+w+w)&32) + (*(p0+w+w+1)&64) + (*(p0+w+w+2)&128);
                // странное лекарство
                // В этой строке нет проверки левого верхнего цикла матрицы что разрушает квадрат из четырех пикселей и тогда алгоритм ставит точку
                ...
                if ( rg0>2 && table_1[rg0]==0 ){
                    ...
                    y_=(w_1+x)/w;  // w_1=w+1;
                    pt.x=(w_1+x)-w*y_; pt.y=y_;  // pt.type=0;  // x+w_1
                    focalPoint.push_back(pt);     // сохраняем в вектор
                }   
                //cout<<(unsigned short)*p0;
                //cout<<" "<<(unsigned short)rg0;
            } // if  
        ...
        if (GrafT) {        
            ...
            // визуализация черным особых точек (фокальных точек)
            for ( n=0; n < sizeAdr; n++ ) { bytes_data[(int)(focalPoint[n].y*w+focalPoint[n].x)]=0; } // читаем из массива векторов
        }
    ...
    int  GGraphBezier::focalPointT1() {
        ...
        unsigned char *bytes_data_1=0, *p1;
        if (GrafT) {                
