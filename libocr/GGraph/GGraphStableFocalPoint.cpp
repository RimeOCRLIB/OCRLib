...
namespace ocr{
  ...
  int  GGraph::StablePoint() {
    ...
    unsigned char *bytes_data_2=NULL;
    if (GrafT) {
    ...
    graphFocalLine(n_RemovLayers);  // n_RemovLayers - количество удаленных слоев (слой это периметр символа).
    int min_RemovLayers=nRemovLayers;
    ...
    OCRPoint *pt_p=(OCRPoint*)&focalPoint[0];
    // (OCRPoint*) приведение типа
    // focalPoint  массив  OCRPoint-ов
    ...
    for ( n=1; n < n_Lay+1; n++ ) { // n=1 !! Величина статуса начинается с единици.
      ...
      for ( m=0; m < sizeAdr; m++ ) {
        ...
        if ( bytes_data[s] == 255 && pt_p[m].data == 0 ) {    // 0 - черное, 255 - белое
          pt_p[m].data=n;
    ...
    if (GrafT) {
      for ( m=0; m < sizeAdr; m++ ) {
        ...
        cout<<"stability="<<(short)pt_p[m].data;
    ...
    /*
     ...
     int MaxStability=0;   int NoMaxStability=0;
     for ( m=0; m < sizeAdr; m++ ) { if (pt_p[m].Stability > MaxStability) { MaxStability=pt_p[m].data; NoMaxStability=m;} }
     */
    ...
    if (GrafT) {
      for ( m=0; m < sizeAdr; m++ ) {
        ...
        if ( pt_p[m].data == MaxStability ) {    // 0 - черное, 255 - белое
  ...
  int  GGraph::StablePointT() {
    ...
    // запуск функции определения фокальных линий букв (графем)
    graphFocalLine(n_RemovLayers-n);  // n_RemovLayers - количество удаленных слоев (слой это периметр символа).
    if (GrafT) { cout<<"Функция определения фокальных линий букв (графем)   focalLine() n_RemovLayers="<<n_RemovLayers<<endl; }
    
    // запуск функции выделения фокальных точек на фокальных линиях (графемах) букв.
    sizeAdr=(int)starArray.size();
    //graphFocalLine(n_RemovLayers);
    if (GrafT) { cout<<"Функция выделения фокальных точек на фокальных линиях букв   focalPoint(focalPoint)   sizeAdr="<<sizeAdr<<endl; }
    /**/
    ...
    for ( m=0; m < sizeAdr; m++ ) { if (pt_p[m].data > MaxStability) { MaxStability=pt_p[m].data; NoMaxStability=m;} }
    if (GrafT) {
  ...
  /*
   ...
   vector<OCRPoint>*focalPoint=vector<OCRPoint>::create(); // вектор
   // запуск функции выделения фокальных точек на фокальных линиях (графемах) букв.
