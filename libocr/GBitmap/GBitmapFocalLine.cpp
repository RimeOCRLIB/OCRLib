...
namespace ocr{
  ...
  int  GBitmap::focalLine(int n_RemovLayers) {
    ...
    if(n_RemovLayers==0) return 0;
    
    // ограничения количество удаленных слоев.
    if (n_RemovLayers < 0) n_RemovLayers = 0;   if (n_RemovLayers > 32) n_RemovLayers = 32;
  ...
  int  GBitmap::focalLineT(int n_RemovLayers) {
    ...
    // цикл перебора всех возможных значений матрицы (3*3-1)
    for (int index=0; index < 256; index++){
      rg0=index;
