...
void GBitmap::packImg32V(int invert){
  ...
  data_size_p32=data_size/32-1; /// размер упакованного массива integer. 1 integer - 32 пиксела изображения
  ...
  int regLimit=31;
  if(ncolumns<31)regLimit=ncolumns;
    
  for( y=0; y < data_size_p32; y++ ){
    reg=0;
    d=ncolumns*stepX+stepY;
    stepX++;
    if(stepX==nrows){stepX=0;stepY+=32;}
