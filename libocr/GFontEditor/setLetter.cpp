...
void GFontEditor::setLetter(){
    ...
    for (xml_node row = doc.child("glyph"); row; row = row.next_sibling("glyph")){
        ...
        size_t indBase=logicProcessor->fontGMap->getHKey(name,REC_LETTER_UNI_UTF);
        
        TString strT; logicProcessor->fontTable->getTStr(indBase,strT);
        if(strT.size()>7){
            DR("name="<<strT[REC_LETTER_UNI_UTF]<<" OCRKey = "<<strT[REC_OCR_KEY]<<endl)
            letter->OCRKey=strT[REC_OCR_KEY];
        }else{
        ...
        if(logicProcessor->mainLetterTableUni.find(letter->name)!=logicProcessor->mainLetterTableUni.end()){
            letter->OCRKey=logicProcessor->mainLetterTableUni[letter->name].OCRKey;
            letter->stackFlag=logicProcessor->mainLetterTableUni[letter->name].stackFlag;
        ...
        aliKali->testLetter=(int)aliKali->letterCount()-1;
        //setMaskFromBase();
...
void GFontEditor::setBase(){
    ...
    for (xml_node row = doc.child("glyph"); row; row = row.next_sibling("glyph")){
        ...
        //    letter->OCRKey="S";
        //}else{
        int indBase=(int)logicProcessor->fontGMap->getHKey(name,8);
        TString strT;
