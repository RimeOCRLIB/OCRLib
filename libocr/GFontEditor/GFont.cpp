...
namespace ocr {
    ...
    return;
		...
		for (xml_node row = resultSet.child("rec"); row; row = row.next_sibling("rec")){
            ...
            letter->letterID=atoi(row.attribute("nb").value());
    ...
    void GFont::writeGFontXML(){
        ...
        for(i=0;i<letterCount();i++){
            ...
            book.append_attribute("nb")=letter->letterID;
    ...
    GBitmap* GFont::drawLetterPict(int in){
        //if(letterSet[in]->pictFlag==0){
        GLetter *letter=getLetter(in);
        GBitmap *img=letter->mask128.unPackMask();
        img->scaleFast(0.4);
    ...
    GBitmap* GFont::drawLetter(unsigned int in){
    ...
    void GFont::setFocalLine(){
        ...
        for(int i=0;i<c;i++){ if(i%500==0)cout<<i<<" ";
         ...
         cout<<"Done set OCRFocalLineBezier in font database."<<endl;
        
    }

    static bool  sortFocalLine(const OCRFocalLineBezier &a, const OCRFocalLineBezier &b){
        int a_=a.len;
    ...
    void GFont::setFocalLineInLetter(uint index){
            ...
            for(int i=0;i<c;i++){
                OCRFocalLineBezier &line=focalLine[i];
                //if(line.len>maxSide){
