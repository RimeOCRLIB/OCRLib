...
#include "../GFontEditor/GContour.h"
#include "../GGraphBezier/GGraphBezier.h"
...
///класс содержит данные буквы языка и методы обращения к ее графическим
///особеностям
class GLetter {
private:
  GLetter(void);
...
public:
  ...
  GBitMask128 maskOriginal; ///габаритная маска оригинального исходного
                            ///изображения буквы
  ...
  GStr<OCRPointFloat> *focalPoint; ///массив топологических ключевых точек буквы
                                   ///(фокальные точки)
  vector<OCRFocalLineBezier>
      focalLine; ///массив линий соединяющих топологические ключевые точки буквы
                 ///(фокальные линии)
  GStr<OCRBox>
      *openType; ///массив габаритов всех составляющих букву элементов OpenType
  OCRPointFloat
      *fPoint; ///указатель на массив топологических ключевых точек буквы
  OCRFocalLineBezier *fLine; ///указатель на массив фокальных линий буквы
  int pictFlag; ///флаг наличия изображения в букве
  int dX, dY; ///<смещение координат центра буквы относительно изображения буквы
              ///<(mask128)
  string name;
  string OCRKey;
  uint letterID; // uniq letter ID number in database (UNIX timestamp)
  int OCRIndex;
  int correlation;     ///< value of correlation
  int selfCorrelation; ///< test value of self correlation
  int codeSpace; ///< test value of test value of code space between this letter
                 ///< and all letters in letter base
  int Character;
  int vectorLimit;
  int stackFlag;
  int OCRStatus;
  int letterW; ///габаритная ширина буквы в строке без учета огласовок и
               ///подписных букв
  ...
  int y0; //верхний габарит строки относительно центра изображения буквы
          //(mask128)
  int y1; //нижний габарит строки относительно центра изображения буквы
          //(mask128)
  int textLineSize; // size of text line in text in pixels
  int newLetter;
  int allLetterCount;  ///<количество букв с таким значением Юникод в корпусе 400mb
  int GFontSize;
  int lineCount;
  ...
  void maskNormalisation(); ///устанавливает высоту всех масок кратной 2
                            ///Необходимо для функций ASM
  void setLetterDimensions(); ///Установление границ и координат центра буквы
                              ///относительно строки
  void eraseAllMask() {
  ...
  void
  eraseMaskContent(int maskId); ///стирает содержимое маски из изображения буквы
  ...
  GBitmap *drawPict32(); ///создает изображение буквы с признаками в сжатом по
                         ///32 бита GBitmap
  ...
  ulong mask32Count() { return mask32Vector->size(); }; ///<количество признаков
  ulong fPointCount() { return focalPoint->size(); }; ///<количество признаков
  void pop_backMask() { mask32Vector->pop_back(); }; ///<стирает последнюю маску
};
...
public:
  ...
  int letterIndex;
  int lineIndex;
  uint letterID; //уникальный ID номкр буквы в базе
  int letterW;
  int letterH;
  int lineH;
  int maxY;
  ...
  vector<int> dIndex; // вектор индикатора наличия разделителя слогов после
                      // буквы гипотезы
  uint length; // длина фразы найденой в словаре
  ...
  void setCenter(); //устанавливает ценр буквы по габаритам и персчитывает
                    //координаты масок признаков
  void drawPict32(GBitmap *outBitmap32, int dx, int dy,
                  int mode); ///создает изображение буквы с признаками в сжатом
                             ///по 32 бита GBitmap
...
public:
  ...
  // многоугольника вертикальной колонки (с добавлением бордюра вокруг блока
  // текста). Xfg0,Xfg1, Yfg0,Yfg1;  тоже что и  xfg0,xfg1, yfg0,yfg1 только не
  // вычитается border ( в функции data_normalisation ).

  // xbr0,xbr1,ybr0,ybr1; зазор (бордюр) между многоугольниками вертикальной
  // колонки. frameFlag  флаг номера колонки (значение флага - номер колонки) 0
  // не определено. 1, 2, 3,... номера вертикальных колонок на развороте.
...
public:
  ...
  // wordOCR(int _x,int _y,int _x1,int
  // _y1){x0=_x;x1=_x1;y0=_y;y1=_y1;w=_x1-_x;h=_y1-_y;};
  int x0, y0, x1, y1, xp0, yp0, xp1, yp1, xp, yp, xt0, yt0, xt1, yt1, w, h;
  int xv0, yv0, xv1, yv1;
  OCRPointFloat center;
  string name;
  string Wylie;
  string html;
  string delimeter;
  ...
  // if(x0==wt.x0&&x1==wt.x1&&y0==wt.y0&&y1==wt.y1&&name==wt.name) return 1;
  // //&&w==wt.w&&h==wt.h&& return 0;
  //}

  bool operator<(wordOCR &right) { return correlation < right.correlation; };
...
public:
  ...
  int x0, y0, x1, y1, w, h;
  int stringFlag; /// флаг cтроки  =0 (=1), cтрока состоит из линии (букв).
                  /// //mark flag for pecha border and empty string
  int NLetter; /// примерная длина строки (оценка)   //letter count
  int LimY0; /// верхний лимит строки               //upper y limit
  int LimY1; /// нижний лимит строки                //doun y limit
  int MeanSA; /// средее, по всей строке, расстояние между центрами масс букв
              /// //middle letter space
  int vLim; /// нижний лимит длинны вектора для данной строки //vector length
            /// limit for string
  ...
  int operator==(stringOCR &wt) {
    if (name == wt.name)
      return 1; //&&w==wt.w&&h==wt.h&&
    return 0;
  }
  vector<wordOCR> wordArray; // main point. here is information about every
                             // letter correlation
  ...
  void
  set_id(string &srcStr); ////на вход функции приходит строка ответа базы данных
                          ///с расставленными слогами. как выход функции мы
                          ///получаем строку в которой сохранена пунктуация
                          ///исходной строки с добавлением пунктуации словаря
  // string html_report(string &srcStr);
  void clearOCR(); //стирает результаты распознаваниея в строке. Устанавливает
                   //имена в wordArray как (*)
