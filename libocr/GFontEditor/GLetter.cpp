...
void stringOCR::set_id(string &desTString){
    ...
    while(startIndex<count){
        ...
        DC("wordArray["<<startIndex<<"].name="<<wordArray[startIndex].name<<
...
GLetter::GLetter(){
    ...
    focalPoint=GStr<OCRPointFloat>::create(); //128
    mask32=(GBitMask32*)mask32Vector->dataPtr();
    fPoint=(OCRPointFloat*)focalPoint->dataPtr();
    //focalLine=GStr<OCRFocalLineBezier>::create(128);
    name="";
    OCRKey="";
}
...
void GLetter::reloadMask(){
    ...
    //destImg->savePNG("/_Image2OCR/31mask.png");
...
void GLetter::writeToStr(TString *st){
    ...
    d1[0]= (uint)letterIndex;                                ///letter index in base
    d1[1]= stringNum;
    d1[2]= recordFlag;
    d1[3]= recordStatus;
    d1[4]= onOffRate;
    d1[5]= keyCount_;
    d1[6]= contourCount;
    d1[7]= (uint)mask32Count();                                ///<количество признаков
    ...
    st->push_back((char*)focalPoint->dataPtr(),(int)focalPoint->size()*(int)sizeof(OCRPointFloat));
...
void GLetter::readFromStr(TString *st){
    ...
    newLetter           =d1[35];  
    letterID     =d1[36];
    allLetterCount      =d1[37];             ///<количество букв с таким значением Юникод в корпусе 400mb
    GFontSize           =d1[38];
    lineCount           =d1[39];
    ...
    if(st->size()>5){
        ...
        focalPoint->push_Ptr( (OCRPointFloat*)&keyStr[0],(uint)keyStr.size());
        fPoint=(OCRPointFloat*)focalPoint->dataPtr();
    }
    ...
    if(st->size()>7){
        ...
        for(int i=0;i<keyStr.size();i++){
            ...
            OCRFocalLineBezier line;
            keyStr.get(&s_, i);
            line.readFromStr(*s__);
            focalLine.push_back(line);
        }
        fLine=(OCRFocalLineBezier*)&focalLine[0];
    }
...
void GLetter::setLetterDimensions(){
    ...
    //проверяем есть ли оригинальное изображение буквы
    GBitmap *img=mask128.unPackMask();
    GBitmap *img_1,*img_2;
    //img->savePNG("/_Image2OCR/1.png");
    DC("/aliKali[0]["<<index<<"]->y0="<<y0<<"aliKali[0]["<<index<<"]->y1="<<y1<<" letterW="<<letterW
       <<" letterH="<<letterH<<" yMask="<<mask128.yMask<<" xMask="<<mask128.xMask<<endl);
    
   
    if((letterH/2+y0<-7&&abs(y0-y1)<letterH/5)||abs(y0-y1)>letterH){    //проверяем габариты строки
        y0=-letterH/2;
        y1=letterH/2;
    }
    DC(" aliKali[0]["<<index<<"]->y0="<<y0<<"aliKali[0]["<<index<<"]->y1="<<y1<<" letterW="<<letterW<<" letterH="<<letterH<<endl)

    
    int xOld=mask128.xMask;
    ...
    maskOriginal=mask128;
    //img_1->savePNG("/_Image2OCR/__img.png");
    
    //установление габарита распознавания буквы. Это габарит основных признаков буквы без выностных элементов.
    //Для некоторых букв этот габарит установлен вручную, для остальных устанавливаем габарит распознавания программно
    //if(dX==0&&dY==0){
        ...
        //img_2->savePNG("/_Image2OCR/__img_h.png");
        ...
        dX=x0+1+letterW/2-mask128.mW/2;
        img_2->destroy();
    //}
...
/*
    for(int a=0;a<mask32Count();a++){
            DR("/////   aliKali[0]["<<letterIndex<<"]->mask32["<<a<<"].xMask="<<mask32[a].xMask<<endl)
            mask32[a].xMask-=dX_;
            DR("/////   aliKali[0]["<<letterIndex<<"]->mask32["<<a<<"].yMask="<<mask32[a].yMask<<endl)
            mask32[a].yMask-=dY_;
    }
*/
    reloadMask();
...
void OCRMatch::copyData(GLetter *letter){
    ...
    letterIndex=(int)letter->letterIndex;
