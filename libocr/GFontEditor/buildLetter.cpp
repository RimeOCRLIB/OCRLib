...
void GFontEditor::buildGFontFromFont(){
	...
	vector<string>fontData;
	explode("\n",data,fontData);
	//string fontName="Kedage";
    //string fontName="Siddhanta";
    string fontName="Yagpo Tibetan Uni";
...
void GFontEditor::buildGFontFromRTF(){
    ...
    if(firstLineFlag){
        ...
        BuildLine:;
            ...
            textLineImg->savePNG("/_1.png"); //exit(0);
            ///set line coordinates in page	
 ...
 if(drawLineFlag){    
    ...
    BuildLetter:;
        ...
        GBitmap *img128=bMap->detectRegion(5,&x0_,&x1_,&y0_,&y1_,1);
         if(mode==1){
            letterName="/_LETTERS/"+substr((int)uniStack.find(":|:")+3,(int)uniStack.size(),uniStack)+".png";
            img128->savePNG(letterName);
            bMap->savePNG("/-3.png");
