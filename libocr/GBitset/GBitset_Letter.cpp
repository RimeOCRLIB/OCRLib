...
*/
       ...
       if(NStrok){
         for ( y=0; y < NStrok*4 && y<nrows-4; y+=4 ){
				 ...
				 strArray[0][d].NLetter=0;    // примерная длина строки (оценка)
				 d++;
...
//  strArray->resize(NStrok);
    if( NStrok >=1 ){
         ...
         for ( d=0; d < NStrok && d<nrows-4; d++ ){
                 ...
                 // vLim минимальная длина вектора
...
void GBitset::stringNormalisation(){
    ...
    // Добавлено вычисление средней высоты строки 2 марта 2019.
    int HStr_=0; DeltaNstr=0;
    for ( n=0; n < NStrok; n++ ){
        strArray[0][n].y1=strArray[0][n].y0+HStrOut[n];  ///cout<<HStrOut[n]<<" ";
        HStr_+=HStrOut[n];
    }
    if(n!=0) DeltaNstr=HStr_/n;
    ///cout<<"   DeltaNstr="<<DeltaNstr<<endl;
