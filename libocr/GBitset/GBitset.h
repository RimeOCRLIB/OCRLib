...
protected:
  ...
  GBitset(GBitset *ref);
  GBitset(GBitmap *ref, short stepW=1, short stepH=1,short flipV=0);
  GBitset(GBitset *ref, int border);
  GBitset(GBitmap *ref, GBitmap *ref1);
public:
  virtual ~GBitset();
  void destroy(void);
  /** @name Construction. */
  ...
  static GBitset* create(GBitset *ref)
  ...
  static GBitset* createResize(GBitmap *ref, short stepW, short stepH, short flipV)
  {return new GBitset(ref,stepW,stepH,flipV);}
  ///(deprecated)
  static GBitset* createLevel(GBitmap *ref)  //@@@?
  ...
  static GBitset* create(GBitset *ref, const int border)
  ...
  void initLevel(GBitmap *ref);
  /** Initializes this GBitset content of the GBitset #ref#.*/
  GBitset* operator=(GBitset *ref);
...
// Подпрограмма управления последовательностью выполнения всех подпрограмм.
  ...
  void pageStringDetectorOCR(vector<OCRBox>&strArray_);
