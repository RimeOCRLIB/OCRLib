...
void GBitset::init(GBitmap *ref, short stepW, short stepH, short flipV) {
  border = 64;
  ...
  if(stepH < 1) stepH = 1;
  if(stepW < 1) stepW = 1;
  int nrowsRef = ref->rows() ;
  int ncolumnsRef = ref->columns();
  init(nrowsRef / stepH, ncolumnsRef  / stepW, border);  //функция инициализации изменяет габариты. nrows+=border ncolumns+=border
  bool *row = bits_data;
  bool *p, *p1;
  int   dH, dW;
  int   h = ref->rows();
  int   hLimit = nrowsRef + border;
  int   wLimit = ncolumnsRef + border;
#ifdef DEBUGLVL_GREY
  ...
  if(flipV) {
    for(int n = border + 1; n < hLimit; n++) {
      p1 = row + n * ncolumns;
      dH = h - (n - border) * stepH;
      for(int m = border; m < wLimit; m++) {
        p = p1 + m;
        dW = (m - border) * stepW;
        if(ref->get(dW, dH) < 125) {
          *p = 1;  // BitVec[row+n*ncolumns+m]=1;
#ifdef DEBUGLVL_GREY
          drawData->put(m, n, 0);  // DM(1);  // 0-черное, 255-белое ?
#endif
#ifdef DEBUGLVL_DRAW
  ...
  for(int x = 0; x < data_size; x++) {
    BitsBuf[x] = bits_data[x];
  }
  // #define B BitsBuf

}  //__________________________________________

void GBitset::initLevel(GBitmap *ref) {
  srcW = ref->columns();
  ...
  for(int x = 0; x < ncolumns * nrows; x++) {
    BitsBuf[x] = bits_data[x];
  }
  // #define B BitsBuf
  draw->destroy();
