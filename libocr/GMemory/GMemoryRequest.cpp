...
void GMemory::select(string&tableName,vector<int> &field, string &query, int searchMode,vector<TString> &result,int limit){
    ...
    switch(searchMode){
        ...
        case TEXT_SEARCH:{
            //результатом поиска является массив текстовых строк
            int searchField=field[0];
            //create index for requested field.
            indexRecord *indRec=createIndex(rec,searchField,searchMode);
            if(limit==1){
        ...
        case FULL_TEXT_SEARCH:{
            //результатом поиска является массив ссылок типа rLink на текстовые данные
            int searchField=field[0];
            //create index for requested field.
            indexRecord *indRec=createIndex(rec,searchField,searchMode);
            ...
            if(limit==1){
                ...
                if(indexInVector.size()){
                    for(int i=0;i<indexInVector.size();i++){
                        ...
                        st=(rLink*)&s[0];
                        //cout<<endl<<indexInVector[i]<<" i:"<<st->index<<" "<<(short)st->field<<" "<<(short)st->offset<<endl;
                        TString sT;
                        sT.push_back((char*)st, sizeof(rLink));
                        result.push_back(sT);
                    }
            ...
            //поиск подстроки в строке. Возвращает массив ссылок на данные текстового вектора.
            int searchField=field[0];
            GVector *v=rec.data;
