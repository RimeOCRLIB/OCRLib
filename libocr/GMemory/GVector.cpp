...
void GVector::init(string &path) {
    ...
    parent=0;
    //cout<<" dataMFile->size()="<<dataMFile->size()<<" path:"<<path<<endl;
    ...
    if(dataMFile->size()==0) {  //новый файл GVector
        ...
        *recordCount=1;
        ...
        innerData=(ulong *)(data);
        if(innerData[0] !=0xfffffffffffffffa) {
            // innerData[0]=0xfffffffffffffffa;
...
void GVector::push_back(string &str) {
    ...
    if(*recordArraySize < *recordCount + 1) {
        cout<<"resize GVector index"
            <<" recordArraySize="<<*recordArraySize<<" recordCount="<<*recordCount<<" *poolSize="<<*poolSize/1000000<<endl;
...
void GVector::setSize(ulong newRecordCount, ulong newSize) {  // datasize указывается в байтах.
    ...
    // cout<<"*poolSize="<<*poolSize/1000000<<" newSize="<<newSize/1000000<<" *dataSize="<<*dataSize/1000000<<"
    // newRecordCount"<<newRecordCount*4/1000000<<endl;
...
void GVector::getStr(string &str, cstr name) {
    ...
    if(indexRecord > *recordCount || indexRecord==0) {
        str="";
        return;
    }
    size=(ulong *)(data + index[indexRecord]);
...
int GVector::getInt(ulong indexRecord) {
    ...
    ulong size;
    if(indexRecord > *recordCount) { return 0; }
...
void GVector::getTStr(ulong indexRecord, TString &str) {
    ...
    // if(str.len>200000){cout<<"error"<<indexRecord<<endl;str.len=0;}
...
void GVector::getTStrData(ulong indexRecord, TString &str) {
    ...
    uint *ptr=(uint *)(data + index[indexRecord]);
    ptr +=2;             //пропускаем размер записи TString в GVector
    str.len=*(ptr);   // cout<<" str->len="<<str->len;
    str.index=ptr + 1;  // cout<<" str->index[0]="<<str->index[0]<<" str->index[1]="<<str->index[1];
...
void GVector::putTStr(ulong indexRecord, TString &st) {
    ...
    if(str.size() < st.sizeData + st.len * 4 + 8) { str.resize((st.sizeData + st.len * 4 + 8) * 2); }
...
/**возврат указателя на Ptr, дочерний GStr или GVector по первой букве имени */
void *GVector::operator[](const char *name) {
    const char c=name[0];
    return (void *)(data + index[c] + 8);
}
...
string TString::operator[](uint i) {
    ...
    if(size_ && size_ < 100000000) {
        ...
        if(size_ !=0) {
            cout<<endl
                <<" @i="<<i<<" index["<<i + 1<<"]="<<index[i + 1]<<" index["<<i<<"]="<<index[i]<<"size_="<<size_
                <<" l:"<<len<<endl;
        }
    }
    return str;
}
/** @bref самая быстрая и не безопасная операция чтения данных TString. Указатель на данные верен только до того момента как
    существует и не модифицируется TString и GVector или другой контейнер в котором записана TString.
*/
void TString::getPtr(char **p, uint i, int *size) {
...
void TString::exportHex(string &str) {
    str="";
    for(int i=0; i < len; i++) {
        string s;
        getStr(s, i);
