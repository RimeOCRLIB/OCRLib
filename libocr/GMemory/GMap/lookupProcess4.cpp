...
bool GMap::lookupProcess4(ulong n,ulong oldw,ulong w){   // int *xMaxStart, int *xMaxEnd
    ...
    uint cnt0Max, cnt1Max;   // максимальное значение счетчиков нулей и единиц  // длина,
    ...
    if ( dlt == 0 ) { // ЧЕТКИЙ поиск.
        ...
        for(x=0;  x < size_BufTxt;  x++) { 
            ...
            if ( BufTxt[x] > 0 ) { 
                ...
                if ( cnt1 > cnt1Max ) { cnt1Max=cnt1; xMaxEnd=(int)x; }
                // сброс счетчика  cnt1 
                cnt1=0;
            } // else
        } // x
        ...
        for(x=0; x < size_BufTxtD; x++) { // size_BufTxt-dlt;
            ...
            for(int m=(int)x; cnt0 < dlt1 && m < size_BufTxt; m++) { // dlt1=dlt+1 иначе не отрабатываются единичные дырки в не четком фрагменте текста
            ///for(m=x; cnt0 < dlt1; m++) {
            ...
            // нахождение максимума количества единиц cnt1 в распознаваемом тексте
            if ( cnt1 > cnt1Max ) { cntDltEnd=cntDlt; cnt1Max=cnt1; xMaxStart=(int)x; }// > остаточный хвост нулей dltEnd в начале не четкого фрагмента текста
...
///            cout<<x<<"."<<cnt1<<"  ";
        ...
        xMaxEnd=(int)(xMaxStart + cnt1Max + dlt - 1); // +dlt-1  // -cnt0Max-2
...
//------------------------------------------------------------------
    ...
    ulong  middleMatchText=(xMaxEnd + xMaxStart)/2-2; // сдвиг на -2 (половину unsigned long)
    ulong  middleMatchDict;
    ...
    if ( dlt == 0 ) { // ЧЕТКИЙ поиск.
        ...
        for(x=oldw;  x < w;  x++) { 
            ...
            if(*(ulong*)(dictionary_data+x)!=*test){  // проверка совпадения 4х подряд идущих букв
                ...
                xMaxEnd=(uint)middleMatchText+x_;
                ...
                xMaxStart=(uint)middleMatchText + x_+ 1;
