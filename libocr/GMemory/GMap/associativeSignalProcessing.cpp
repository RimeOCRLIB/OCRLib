...
/*
 Безразмерный быстрый не четкий поиск. Ноябрь 2018
 
 Идея заключается в комплексировании полезных свойств не четкого поиска и высокого быстродействия хешированного четкого поиска.
 На самом деле словарь фраз не четкого поиска (длинна фразы max 128 букв или сколько сделаем) это словарь окружения коренного слова при его поиске.
 
 Предварительно хешируем по коренным словам словарь фраз не четкого поиска.
 В ячейках хеш таблици храним списки адресов фраз словаря не четкого поиска, в которых встречается данные коренные слова.
 
 В место "словаря фраз не четкого поиска" можно использовать весь корпус текстов, а в место "текста" фразу поискового запроса.
 
 Как это работает ?
 
 - берем коренное слово (коренное слово как часть фразы текста-запроса).
 - считаем его хеш
 - проверяем предпросчитанную хеш таблицу и из ячейки получаем список адресов фраз в которых встречается данное коренное слово.
 По этому линейно читаем уже не весь словарь не четкого поиска (это занимает несколько секунд),
 а только те адреса фраз в котором встречается данное коренное слово, и это существенно быстрее.
 Дальше по выбранным фразам не четкий поиск работает как обычно.
 Объем хеш таблицы соответствует числу разных коренных слов в языке. Таблица состоит из ячеек переменной длинны.
 
 Если нужно проверить несколько коренных слов как часть фразы текста ?
 Для каждого коренного слова получаем не зависимые списки адресов фраз в которых встречается данное коренное слово и считаем их пересечения.
 Списки получаем с помощью хеш таблицы, так как описано выше. Пересечение списков несколько коренных слов получаем с помощью таблицы,
 состоящей из байт. Максимальное количество пересечений 255.
 
 Для этого проверяем таблицу пересечений по всем проверяемым коренным словам по соответствующим им адресами фраз словаря:
 - обнуляем таблицу пересечений.
 - читаем таблицу пересечений по спискам адресов фраз 1ого коренного слова и прибавляем единицу к соответствующим байтам таблицы.
 - читаем таблицу пересечений по спискам адресов фраз 2ого коренного слова и прибавляем единицу к соответствующим байтам таблицы в
 которых уже записана единица.
 - читаем таблицу пересечений по спискам адресов фраз 3ого коренного слова и прибавляем единицу к соответствующим байтам таблицы в
 которых уже записана двойка.
 - когда читаем таблицу пересечений по спискам адресов фраз последнего коренного слова, то проверяем соответствующие байты таблицы и
 запоминаем адреса только тех байт, в которых оказалось максимальное число. Это число равно числу проверяемых коренных слов минус единица.
 Эти и есть адреса фраз в которых встречаеются данные коренные слова т.е. их пересечения.
 
 ДОПОЛНЕНИЕ
 Коренные слова можно найти следующим образом.
 Можно выполнитьь "све на все" словаря и выбрать группы фраз с одним устойчивым словом на всю группу.
...
void GMap::associativeSignalProcessing(string&text){ 
    ...
    Результатом выполнения функции является:
      ...
      +unsigned short *BufM;          // зеркальный массив BufM 128кб, фиксированного размера ALPHABET_SIZE*2=sizeof(unsigned short)
