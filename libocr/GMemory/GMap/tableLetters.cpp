...
void GMap::tableLetters(){
    ...
    //   clusters_data в которой записаны все адреса пары букв по которым она встречактся в тексте. Происходит выборка всех адресов пары букв и
    //   работа со структурой для данного кода пары букв заканчивается.    
...
///    int  *clusters_data
    ...
    // +1 т.к. нулевая ячейка массива не используется
    clusters_data_vector->reserve(clusters_size);    // запрос памяти c обнулением   
    ...
    for(x=0; x < text_size1 ; x++) { // text_size1 размер массива распознаваемого текста без переводов каретки Enter  // nLetterPT
        ...
        bf=BufLpT[ds];        // количество повторений данной пары букв во всем распознаваемом тексте
        ...
        if ( bl == 0 && bf > 1 ) {  
            ...
            bl=letters_data[ds]=-n;     // в массив letters_data на позицию "ds" обновляем код "-n"   //  -(n+bf);
    ...
    for(x=0; x < text_size1; x++) { // letters_size=nLetterPT количество разных пар букв в распознаваемом тексте        
      ...
      if ( bl != 0 ) {
        ...
        if ( bl > 0 ) { 
            ...
            uint cnt=clusters_data[blp+1];   // считываем значение счетчика адресов,  +1 положение счетчика свободных адресов
