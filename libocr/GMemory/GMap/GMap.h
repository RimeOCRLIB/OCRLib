...
namespace ocr{
    ...
    //для поиска по большому корпусу текстов кодируем все тексты на основании словаря слов и фраз.
    //составляем индекс вхождений слов в корпусе для каждого слова словаря.
    //поиск фразы запроса осуществляем только в тех фразах в которых есть более половины слов запроса.
    
    //для поиска результатов распознавания каждую букву представляем как совокупность протобукв (графических кластеров)
    //для каждой протобуквы на основании графической разности пар букв известен вес этой протобуквы в букве
    //поиск результата распознавания ведем в словаре составленном из пар слов словаря (пять-десять слогов)
    //записанных в виде групп протобукв с учетом их веса в образовании буквы и слова.
    ...
    public:
        ...
        void processKeyWordlist(string &key, ulong index,uint field);
        ...
        void getTextKey(string &key,vector<dictKey>&searchResult, int mode);
        ...
        // Проверяется наличие совпадения (четкого или нечеткого) слова с фрагментом фразы словаря.
        void tableProcessingMatch(wstring &key,vector<ulong>&searchResult, uint limit);
        ...
        // Проверяется совпадение (четкое или нечеткое) частей фразы из текста, с частью фразы словаря.
        void tableProcessingText(wstring &key,vector<dictKey>&searchResult, uint limit);
    ...
    private:
        ...
        uint constantPhrase;    // минимальная длина части фразы словаря, которую имеет смысл проверять как непрерывную.(длина ключа)
