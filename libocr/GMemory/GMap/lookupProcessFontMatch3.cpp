      ...
      if ( bl != 0 ) {
        ...
        // заполнение массива адресами (позициями) пар букв в распознаваемом тексте, встречающихся более одного раза
            ...
            for(m=0; m < cnt; m++) {
                ...
                DD(" c"<<startClusterChain<<"="<<Unicode_to_UTF(str)<<" cnt="<<cnt<<" startClusterChain="<<startClusterChain);
                ...
                if(!flag){
                    ...
                    if(m<startClusterChain)continue;
                    BufDictLine[flag]=indexX/2;
                    x0_=letterX[indexX];
                    x1_=letterX[indexX+1];
                    indexStart=x;
#ifdef REMARK
                    str=(wchar_t)BufUp[text_data[indexX/2]]; DD(" c"<<startClusterChain<<"="<<Unicode_to_UTF(str)<<" ");
#endif
                    DD("@@"<<startClusterChain<<"@@"<<m<<" start chain from letter "<<Unicode_to_UTF(str)<<" ");
        ...
        if(!flagNew){  //пара не найдена
              ...
              DD(" startClusterChain="<<startClusterChain<<" flag="<<flag<<endl);
              if(!flag&&startClusterChain){startClusterChain=0;indexStart++;}
              ...
              startClusterChain=0;
              DD("chain break; startClusterChain1="<<startClusterChain<<" count="<<endl);
              flag=0;
