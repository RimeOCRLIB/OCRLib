...
*/
    ...
    for(x=0;  x < size_BufTxt-1&& LPhraseHits < constantPhrase;  x++) { 
        ...
        if ( BufTxt[x] > 0 ) { LPhraseHits++; }  else { LPhraseHits=0; }
                                 ...
                                 // 53c //
    // возвращаем количество совпавших букв фразы с буквами распознаваемого текста по непрерывному фрагменту (порядок букв не учитывается)
    return LPhraseHits;
...
uint GMap::lookupProcess3_T(ulong w, ulong oldw){
    ...
    LPhraseHits=0;
    xMaxEnd=0;
    for(uint x=0;  x < size_BufTxt;  x++) {  //&& LPhraseHits < constantPhrase
        // счетчик длины непрерывного фрагмента фразы словаря совпавшего с непрерывным фрагментом текста, сброс счетчика  LPhraseHits++ .
        if ( BufTxt[x] > 0 ) {
            LPhraseHits++;
            xMaxEnd=x+1;
