...
int GLogicProcessor::textCorrector(vector<OCRMatch>&dLine, string &mainString){
    ...
    explode("@", mainString, lineText);  //cout<<" mainString="<<mainString;
    int indexText=0;
    mainString="";
...
void GLogicProcessor::grammarCorrector(vector<stringOCR>&strArray,
    ...
    int startFlag=0,endFlag=0;  //флаги наличия добавленных слов в начале и конце строки
...
void GLogicProcessor::checkInCorrectionTable(vector<stringOCR> &correctionTable,stringOCR &testStr,string &srcStr){
	...
	//if(!flag){
		...
		string str=out.str();
		testStr.idNumber=strToHash(str.c_str(), (int)str.size());
		//flag=1;
...
string GLogicProcessor::splitEngWord(string &word, int *result){
    ...
    *result=1;
    ...
    bool GLogicProcessor::hasTail(OCRMatch&match){
    ...
    bool GLogicProcessor::isTibDigit(string &name) {
       if(name=="༠" || name=="༡" || name=="༢" || name=="༣" || name=="༤" || name=="༥" || name=="༦" || name=="༧" ||name=="༨" || name=="༩")
           return 1;

       return 0;
   }
vector<wordOCR> GLogicProcessor::explodeOCR(const vector<string> &delimiter, const string &input) {
    ...
    while(start < n) {
        ...
        for(int i=0; i < count; i++) {
            ...
            if(stop < minStop && stop > -1) {
                minStop=stop;
                strItem.delimeter=delimiter[i];
            }
        }
        // start-=1;
        // if(start<0)start=0;
        ...
        // cout<<"n="<<strItem.name<<"/ d="<< strItem.delimeter<<" start="<<start<<" minStop="<<minStop<<"
        // s="<<strItem.delimeter.size()<<endl;
...
vector<stringOCR> GLogicProcessor::explodeOCR(const vector<string> &delimiter, stringOCR &input) {
    ...
    while(start < n) {
        ...
        // cout<<"n="<<strItem.str<<"/ d="<< strItem.delimeter<<" start="<<start<<" minStop="<<minStop<<"
        // s="<<strItem.delimeter.size()<<endl;
        start=minStop + 1;
...
string GLogicProcessor::implodeOCR(const string &delimiter, const vector<wordOCR> &input) {
    string       result;
    unsigned int i;
...
int GLogicProcessor::indexById(vector<stringOCR> &correctionTable, unsigned int in) {
