...
//-----------------------------------------------------------------------------------------
    ...
    GVector *fontTable;             ///main font data table. Read from LETTERS_GRAMMAR.tab
	...
	map<string, uniRecord>YagpoToWylieMap;          ///<map of Yagpo UTF To Wylie and translit conversion
	...
	map<string,uniRecord > mainLetterTableUni;      ///<base letter table for all letter conversion function in Uni key
	map<string,uniRecord > mainLetterTableYagpo;    ///<base letter table for all letter conversion function in Yagpo key
	map<string,uniRecord > mainLetterTableKey;      ///<base letter table for all letter conversion function in typing key
	map<string,uniRecord > mainLetterTableOCRKey;   ///<base letter table for all letter conversion function in OCR
...
//-----------------------------------------------------------------------------------------    
/// @name Language codepage conversion function. All conversion parameter set in inputData structure */	
//@{
	/**  @brief load all conversion map from XML files */
    void  LoadUniToTibetanMapUTF();
	/**  @brief load convrsion map for font from XML files */
    void  LoadFontNameMap( map<string, string>&fontNameMap);
    void  LoadUniToWylieMap();
    ...
    string lineTranscription(string &line, int mode);
	/** @brief convert string from Wylie transliteration to Yagpo UTF (old) */
	string WylieToUniConverter(string &inStr);
	...
	/** @brief convert string from Singhala Unicode font to Sinhala Yagpo precomposed Unicode codepage LETTERS_GRAMMAR.tab */
    ...
    void HTML2UniConverter( string &mainString, string &path);
...
/// @name Dictionary and text processing function
    ...
    void pageSegmentation(vector<OCRMatch>&matchLine,vector<OCRLine>&focalLine,int pageW,int dY);
    ...
    void letterNeighborsSanskrit(vector<OCRMatch>&matchLine,GBitmap* lineImg32,GBitmap* letterAImg,GBitmap* letterBImg);
...
//-----------------------------------------------------------------------------------------
    ...
    void lineTextTranslation(string &destString,int mode);
    void lineTextTranslationSkt(string &destString);
    void lineTextTranslationChn(string &destString);
...
//-----------------------------------------------------------------------------------------
	...
	vector<wordOCR> explodeOCR(const vector<string> &delimiter, const string &input);
	vector<stringOCR> explodeOCR(const vector<string> &delimiter, stringOCR &input);
	string implodeOCR(const string &delimiter, const vector<wordOCR> &input);
