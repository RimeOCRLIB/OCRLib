...
void GLogicProcessor::pageSegmentation(vector<OCRMatch>&matchLine,vector<OCRLine>&focalLine,int pageW,int dY){
    ...
    while(i<matchLine.size()){
        ...
        if(matchLine[i].name=="|"||matchLine[i].name=="།"){
            ...
            for(int j=0;j<focalLine.size();j++){
                OCRLine &line=focalLine[j];
                if(line.lenA<256)break;
                if(abs(line.start.x-line.end.x)<32&&
                   abs(line.start.y-line.end.y)>256
                   ){
