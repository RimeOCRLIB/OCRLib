...
void GLogicProcessor::readDictionary(){
    ...
    //замена подписных тибетских букв строчными
    ...
    //cout_ <<"dict loaded1"<<endl;
        ...
        //на основании бинарной таблицы строим stl:map
        //  field order
        // 0 baseLetter
        // 1 lang
        // 2 letterName
    ...
    for (int i=0; i<fontTable->size();i++){
        uniRecord record; ///base record for all letter conversion function
        TString st;
        fontTable->getTStr(i, st);
        
        record.lang=st[REC_LANG];
        record.letterName=st[REC_LETTER_NAME];
        record.letterYagpoUTF=st[REC_YAGPO_UTF];
        record.OCRKey=st[REC_OCR_KEY];
        record.keyUTF=st[REC_KEY_UTF];
        record.stackFlag=atoi(st[REC_STACK_FLAG].c_str());
        record.letterUniUTF=st[REC_LETTER_UNI_UTF];
        record.uniTypingKey=st[REC_UNI_TYPE_KEY];
        record.Wylie=st[REC_WYLIE];
        record.OCRCount=atoi(st[REC_OCR_COUNT].c_str());
        record.translitRus=st[REC_TRANSLIT];
        
        
        if(record.Wylie=="space"){ //exeption for XML parser
            record.letterYagpoUTF=" ";
            record.letterUniUTF=" ";
        }
        
        mainLetterTableYagpo[record.letterYagpoUTF]=record;
        mainLetterTableUni[record.letterUniUTF]=record;
        mainLetterTableKey[record.uniTypingKey]=record;   //map для конвертации Wylie
        mainLetterTableOCRKey[record.OCRKey]=record;
        if(record.lang=="tib")ASCIToUni[record.Wylie]=record.letterUniUTF;
    }
        //cout<<"k="<<record.letterYagpoUTF<<" v="<<record.uniKey<<" u="<<record.tibUniUTF<<endl;
    ASCIToUni[" "]="་";  //exeption;
...
#define MEM_CHINK_CHAR 1073741824
	...
	for(int i=0;i<sizeText;i++){
		...
		if(c=='\n'){  //if(index>837153)cout_<<index<<" ";
			...
			explode(" ",srcStr,stringItemVector);  
			if(stringItemVector.size()>200){cout_<<"too long line "<<index<<endl; continue;}
...
void GLogicProcessor::loadDictLevelFile(){
    ...
    while(getline(srcInput, srcLine,'\n')){
        ...
        explode(":|:", srcLine,stringItemVector);
