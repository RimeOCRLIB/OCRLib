...
string  GLogicProcessor::spellCheck( string &str_){
	...
	explode(" ",str_,wordList); 
	strResult.resize(wordList.size());
	for(int index=0;index<wordList.size();index++){
		...
		stringItemVector=explodeOCR(delimeterTib,wordList[index]); //stringItemVector now contain all syllabons from string
