...
void GLogicProcessor::renderDictSearchNew(map<vector<int>,ulong>&searchResult,
    ...
    for(int n=0;n<dLine.size();n++){
        ...
        for(int m=0;m<dLine.size();m++){
            ...
            if(dLine[m].xCenter>dLine[n].x0&&dLine[m].xCenter<dLine[n].x1){
                ...
                if(dLine[n].letterW>dLine[m].letterW*1.5){
                    dLine[m].correlation=0;
                }
                if(dLine[m].letterW>dLine[n].letterW*1.5){
    ...
    //убираем знаки препинания внутри стыкованной фразы

    for(int n=0;n<dLine.size();n++){
        if(!dLine[n].correlation)continue;
        if(dLine[n].OCRIndex=='Z'||dLine[n].OCRIndex=='N')continue;
        for(int m=0;m<dLine.size();m++){
            if(!dLine[m].correlation)continue;
            if(m==n)continue;
            if(dLine[m].OCRIndex!='Z'&&dLine[m].OCRIndex!='N')continue;
            if(dLine[m].xCenter>dLine[n].x0&&dLine[m].xCenter<dLine[n].x1){
    ...
    //drawGrapeLine(dLine); exit(0);
    //drawGrapeLine(originalMatch); exit(0);
    
    //по результатам проверки делиметров собираем полученную фразу из букв и знаков препинания (delimeter)
    for(int n=0;n<dLine.size();n++){
        OCRMatch &c=dLine[n];
