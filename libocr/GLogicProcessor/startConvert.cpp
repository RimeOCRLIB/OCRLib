...
string GLogicProcessor::startConvert(){
	...
	if(ocrData=="RTFToUnicode"){
		...
		cout<<"RTFToUnicode inputData.fileList.size()="<<inputData.fileList.size()<<endl;
		string mainString;
		for(int i=0;i<inputData.fileList.size();i++){
			mainString="";
#ifdef COCOA
			RTFtoUniConverter(mainString,inputData.fileList[i]);
#endif
	...
	if(ocrData=="dWylieToYagpo"){
		string mainString;
		if(inputData.data["InputMethod"]!="text"){
			...
			for(int i=0;i<inputData.fileList.size();i++){
				...
				for(int i=0;i<strVector.size();i++){
					...
					mainString+=WylieToUniConverter(strVector[i]);
                    mainString+="\n";
				}	
				writeText(mainString, path);
			}				
    ...
    if(ocrData=="tibTextCorrector"){
        ...
        if(inputData.data["mode"]!="text"){
            ...
            mainString=lineTibetanCorrector(text);
            return mainString;
