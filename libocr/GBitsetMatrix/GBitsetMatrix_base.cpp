...
GBitsetMatrix::GBitsetMatrix(GBitsetMatrix *ref)
  : nrows(0), ncolumns(0), border(0), mask32Count(0),
	bits_per_row(0),data_size(0)
{
  try
...
GBitsetMatrix::GBitsetMatrix(GBitmap *ref, short stepW, short stepH, short flipV)
  : nrows(0), ncolumns(0), border(0), mask32Count(0),
	bits_per_row(0),data_size(0)
{
  try
...
GBitsetMatrix::GBitsetMatrix(GBitmap *ref, short x, short y, short w, short h, short stepW, short stepH, short flipV)
	: nrows(0), ncolumns(0), border(0), mask32Count(0),
	bits_per_row(0),data_size(0)
{
   try
...
GBitsetMatrix::GBitsetMatrix(GBitsetMatrix *ref, int border)
  : nrows(0), ncolumns(0), border(0), mask32Count(0),
	bits_per_row(0),data_size(0)
{
  try
...
void
GBitsetMatrix::init(GBitsetMatrix *ref, int aborder)
{
