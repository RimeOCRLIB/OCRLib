...
#define  c_out1_ inputData.c_out
    ...
    int maxLetterCount=120000;
    int correlationLimit=inputData.ocrCode["OCR_MATRIX_CORRELATION_LIMIT"];
    ...
    for(int index=1; index<letterCount;index++){
        //print=1;if(index!=22354 )continue;
        ...
        if(OCRMode==2){
            ...
            glyph=aliKali->getOCRLetter(index);
            if(glyph==0)continue;
            //int OCRCount=glyph->allLetterCount;
            if(glyph->mask32Count()==0){glyph->destroy();continue;}
            //if(OCRCount<10 || glyph->mask32Count()==0){glyph->destroy();continue;}
        }
        
        
        
        //if(glyph->name!="ཆོ"){glyph->destroy();continue;} print=1;
                ...
                if(pointsCorrelation){
                    ...
                    if(mC>correlationLimit){
                        ...
                        if(matchLine.size()>maxLetterCount){
                            //for(int n=0;n<maxLetterCount-32;n++)cout<<matchLine[n].letterIndex<<" "<<matchLine[n].name<<endl;
                            matchLine.resize(0);
