...
static bool sortFocalLineL(const OCRFocalLineBezier& d1, const OCRFocalLineBezier& d2){
...
void GBitsetOCR::textOCR(vector<stringOCR>&correctionTable,GLogicProcessor *logicProcessor, int mode){
    ...
    //int OCRMode=2; //2- Sanskrit   оптимальная высота строки 39
    inputData.OCRMode=OCRMode;
    
    int rewriteOCRCash;
    rewriteOCRCash=0;
    //((GStr<int>*)inputData.pref)->get(5);
    //((GStr<int>*)inputData.pref)->put(5,0);
    
    ((GBitmap*)inputBitmap)->binarisation64();
    
    GGraphBezier vectorGraph=GGraphBezier(inputBitmap);
    vector<OCRPointFloat>focalPoint;
    ...
    //vectorGraph.drawSVG(focalPoint, focalLine);
    //exit(0);
    ...
    for (int index=(int)strArray[0].size()-1;index>=0; --index){  //for every string
        ...
        //if(index!=3)continue;  //отладочный режим
        ...
        h=inputBitmap->rows();
        inputData.w=w;
        inputData.h=h;  //used in drawGrapeLine
        ...
        if(inputData.data["ocrData"]=="lineOCR"){
            string path=inputData.data["root"]+"/edit/OCR/_OCR/_line.png";
            lineImg->savePNG(path);
        }
        //определяем поворот и при необходимости выравниваем строку
        float p[]={1};
        adaptiveFiltration1(lineImg,p,1);
        ...
        //if(lineImg->bytes_data[0]==255)lineImg->destroy(); //patch
