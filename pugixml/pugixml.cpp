...
namespace pugi
{
	...
	void xml_node::print_xml(xml_writer& writer, const char* indent, unsigned int flags, unsigned int depth)
	{
		if (empty()) return;
...
#ifndef PUGIXML_NO_STL
	void xml_node::print_xml(std::ostream& stream, const char* indent, unsigned int flags, unsigned int depth)
	{
		xml_writer_stream writer(stream);

		print_xml(writer, indent, flags, depth);
	}
#endif
